...
"""
DETR model and criterion classes with video feature conditioning.
"""
import torch
from torch import nn
from torch.autograd import Variable
from examples.baselines.act.act.detr_video.transformer import build_transformer, TransformerEncoder, TransformerEncoderLayer
...
class DETRVAE(nn.Module):
    """ This is the DETR module that performs object detection with video feature conditioning """
    def __init__(self, backbones, transformer, encoder, state_dim, action_dim, num_queries, video_feature_dim=128):
        super().__init__()
        ...
        self.additional_pos_embed = nn.Embedding(3, hidden_dim) # learned position embedding for video feature, state and proprio

    def forward(self, obs, actions=None, video_feature=None):
        ...
        if video_feature is not None:
            video_feature_embed = self.video_feature_proj(video_feature)  # (bs, hidden_dim)
        else:
            video_feature_embed = torch.zeros([bs, self.transformer.d_model], dtype=torch.float32).to(state.device)
        ...
        if self.backbones is not None:
            ...
            hs = self.transformer(src, None, self.query_embed.weight, pos, latent_input, proprio_input,
                                self.additional_pos_embed.weight, video_feature_embed)[0] # (batch, num_queries, hidden_dim)
        else:
            state = self.input_proj_robot_state(state)
            hs = self.transformer(None, None, self.query_embed.weight, None, latent_input, state,
                                self.additional_pos_embed.weight, video_feature_embed)[0]
