#!/bin/bash
# ACT eval on seen/unseen splits, one GPU per checkpoints.
# CHECKPOINT_45/30/15 point to final_model.pt; set CHECKPOINT_DIR_45/30/15 instead to a dir.
...
RESULT_ROOT="${RESULT_ROOT:-runs/act_eval}"/"$(date +%Y%m%d_%H%M%S)"
...
run_act_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -m examples.baselines.act.parallel_eval_act \
...
for tag in "${EVAL_TAGS[@]}"; do
  ...
  (
    run_act_eval "$(gpu_for_tag "$tag")" "$tag" "$run_dir" "$RESULT_ROOT/act_${tag}"
  ) &
  PIDS["$tag"]=$!
done
