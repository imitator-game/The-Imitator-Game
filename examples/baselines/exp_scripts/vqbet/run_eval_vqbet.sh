#!/bin/bash
# VQ-BeT eval on seen/unseen splits, one GPU per checkpoint.
...
RESULT_ROOT="${RESULT_ROOT:-runs/vqbet_eval}"/"$(date +%Y%m%d_%H%M%S)"
...
run_vqbet_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -m examples.baselines.vqbet.parallel_eval_vqbet \
...
for tag in "${EVAL_TAGS[@]}"; do
  ...
  (
    run_vqbet_eval "$(gpu_for_tag "$tag")" "$tag" "$run_dir" "$RESULT_ROOT/vqbet_${tag}"
  ) &
  PIDS["$tag"]=$!
done
