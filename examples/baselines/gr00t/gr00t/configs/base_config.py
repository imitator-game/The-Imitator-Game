...
class Config:
    ...
    def validate(self):
        ...
        if isinstance(self.model, Gr00tN1d6Config):
            ...
            if self.model.tune_diffusion_model and self.model.use_action_head_diffusion_lora > 0:
                raise ValueError(
                    "Cannot enable both tune_diffusion_model and use_action_head_diffusion_lora"
                )

        # Validate precision settings
        if self.training.fp16 and self.training.bf16:
            raise ValueError("Cannot use both fp16 and bf16")
        if self.training.epoch_based_training:
            if self.training.num_epochs <= 0:
                raise ValueError("num_epochs must be greater than 0 when epoch_based_training is enabled")
            if self.training.save_epochs <= 0:
                raise ValueError("save_epochs must be greater than 0 when epoch_based_training is enabled")
        if self.training.enable_online_eval:
            if not self.training.online_eval_env_id:
                raise ValueError("online_eval_env_id must be set when enable_online_eval is enabled")
            if self.training.online_eval_num_episodes <= 0:
                raise ValueError("online_eval_num_episodes must be greater than 0")
            if self.training.online_eval_num_envs <= 0:
                raise ValueError("online_eval_num_envs must be greater than 0")
            if self.training.online_eval_max_episode_steps <= 0:
                raise ValueError("online_eval_max_episode_steps must be greater than 0")
            if self.training.online_eval_steps <= 0 and (
                not self.training.epoch_based_training or self.training.online_eval_epochs <= 0
            ):
                raise ValueError(
                    "Set online_eval_steps > 0 or enable epoch_based_training with online_eval_epochs > 0"
                )
