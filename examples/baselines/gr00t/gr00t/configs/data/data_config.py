...
class SingleDatasetConfig:
    ...
    # LeRobot metadata layout version. "auto" preserves existing behavior and
    # infers the layout from dataset metadata.
    lerobot_version: str = "auto"

    # Controls how the configured language key is populated. "task" preserves
    # the existing behavior and uses the dataset task string directly. When set
    # to "human_desc", the loader maps sim/robot task ids through
    # lerobot_dataset/task_mapping.json and samples a description from
    # task_desc/human_desc.json.
