...
class ShardedMixtureDataset(IterableDataset):
    ...
    def _get_rank_shard_sampling_schedule_from_schedule(
        self,
        shard_sampling_schedule: list[tuple[int, int]],
        num_workers: int = 1,
    ) -> list[tuple[int, int]]:
        """Return the per-rank shard schedule aggregated across dataloader workers."""
        filtered_schedule = []
        for i, shard in enumerate(shard_sampling_schedule):
            shard_owner = i % (self.world_size * num_workers)
            if self.rank * num_workers <= shard_owner < (self.rank + 1) * num_workers:
                filtered_schedule.append(shard)
        return filtered_schedule

    def get_rank_shard_sampling_schedule(self, num_workers: int = 1) -> list[tuple[int, int]]:
        """Return the per-rank shard schedule aggregated across dataloader workers."""
        return self._get_rank_shard_sampling_schedule_from_schedule(
            self.shard_sampling_schedule,
            num_workers=num_workers,
        )

    def get_num_samples_per_epoch(self, num_workers: int = 1) -> int:
        """Return the number of samples yielded by one rank during a single internal epoch."""
        rank_schedule = self.get_rank_shard_sampling_schedule(num_workers=num_workers)
        return sum(
            self.datasets[dataset_idx].get_shard_length(shard_idx)
            for dataset_idx, shard_idx in rank_schedule
        )

    def get_num_samples_for_epoch(self, epoch: int, num_workers: int = 1) -> int:
        """Return the number of samples yielded by one rank for a specific internal epoch."""
        schedule = self.generate_shard_sampling_schedule(epoch=epoch)
        rank_schedule = self._get_rank_shard_sampling_schedule_from_schedule(
            schedule,
            num_workers=num_workers,
        )
        return sum(
    ...
    def generate_shard_sampling_schedule(self, epoch: int | None = None) -> list[tuple[int, int]]:
        """
