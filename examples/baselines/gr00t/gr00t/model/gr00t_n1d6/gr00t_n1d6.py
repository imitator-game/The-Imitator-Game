...
def _mark_only_lora_parameters_trainable(module: nn.Module) -> None:
    """Enable gradients only for LoRA adapter parameters in a wrapped module."""
...
class Gr00tN1d6ActionHead(nn.Module):
    ...
    def __init__(self, config: Gr00tN1d6Config):
        ...
        if config.use_action_head_diffusion_lora > 0:
            self._wrap_diffusion_lora(config.use_action_head_diffusion_lora)
    ...
    def _wrap_diffusion_lora(self, rank: int) -> None:
    ...
    def configure_trainable_parameters(
        ...
        elif self.config.use_action_head_diffusion_lora > 0:
            _mark_only_lora_parameters_trainable(self.model)
        ...
        print(
            f"Use action head diffusion LoRA: {self.config.use_action_head_diffusion_lora > 0}"
        )
    ...
    def set_frozen_modules_to_eval_mode(self):
        ...
        if self.training:
            ...
            if (
                not self.tune_diffusion_model
                and self.config.use_action_head_diffusion_lora == 0
            ):
                self.model.eval()
...
class Gr00tN1d6(PreTrainedModel):
    ...
    def __init__(
        ...
        self.backbone = backbone_cls(
            ...
            use_backbone_lora=config.use_backbone_lora,
            use_llm_lora=config.use_llm_lora,
