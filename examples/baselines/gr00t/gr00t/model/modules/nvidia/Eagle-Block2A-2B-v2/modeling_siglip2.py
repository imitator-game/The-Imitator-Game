    ...
    def split_patch_embeddings_to_windows_with_meta(
        ...
        Returns:
            ...
            win_meta_list:      List[dict] with keys:
                ...
                - win_hw:    effective window size within the original image (h_eff, w_eff)
        """

        # 1. Compute the starting position of each image within the flat tensor
        batch_hw = batch_hw.tolist()
        counts = [H * W for (H, W) in batch_hw]
        starts = [0] + list(accumulate(counts))[:-1]

        # 2. Group by (H, W) and process same-size images together
        ...
        all_meta = []
        # print(size2info)
        # 3. Perform batched unfold + pad for each size group
        for (H, W), info in size2info.items():
            ...
            # 3.2 Compute pad sizes (bottom, right) to make the result divisible by window_size
            ...
            # 3.5 Generate meta: record the effective window size within the original image
            for b, img_idx in enumerate(img_idxs):
                for win_id in range(n_windows):
                    i, j = divmod(win_id, n_w)
                    h0, w0 = i * window_size, j * window_size
                    # Actual ending coordinates within the original image
        ...
        # 4. Concatenate and sort by img_idx + win_xy to restore the input order
        sorted_idx = sorted(
            range(len(all_meta)),
            key=lambda k: (
                all_meta[k]["img_idx"],
        ...
        for meta, win in zip(win_meta_list, all_windows):
            ...
            # Keep only patch tokens that truly come from the original image
        ...
        # If you need a single tensor, you can cat once more:
        all_tokens = torch.cat(windows_list, dim=0).unsqueeze(
            0
        )  # shape (sum(valid_num), C)

        # 1. Recompute each image's starting position in the original flat tensor
        counts = [H * W for H, W in batch_hw]
        starts = [0] + list(accumulate(counts))[:-1]
        total_patches = sum(counts)

        # 2. Build the mapping: mapping[orig_idx] = new_idx
        mapping = [None] * total_patches
        offset = 0  # cursor over the all_tokens dimension
        ...
        for meta in win_meta_list:
            ...
            # Compute the mapping for all patch tokens in this window that truly come from the original image
            for u in range(h_eff):
                for v in range(w_eff):
                    # Original flat coordinates
                    orig_idx = base + (h0 + u) * W + (w0) + v
                    # Position in all_tokens: flattened row-major within this window's segment
                    p = u * w_eff + v
                    mapping[orig_idx] = offset + p

                # After the window ends, advance offset by the window's effective token count
            offset += h_eff * w_eff
