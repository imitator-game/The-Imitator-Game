...
def _lazy_import_torchcodec():
    ...
    try:
        ...
        raise ImportError(f"torchcodec is not available: {exc}") from exc
...
def _check_backend_available(backend: str) -> tuple[bool, str | None]:
    ...
    if backend == "torchcodec":
        ...
        return True, None
    return False, f"unknown video backend '{backend}'"


def _is_backend_available(backend: str) -> bool:
    return _check_backend_available(backend)[0]
...
def resolve_backend(video_path: str, requested_backend: str) -> str:
    ...
    if not _is_backend_available(requested_backend):
        unavailable_reason = _check_backend_available(requested_backend)[1]
        if requested_backend == "torchcodec":
            raise ImportError(f"Video backend 'torchcodec' is required but unavailable: {unavailable_reason}")
        for fallback in _BACKEND_FALLBACK_ORDER:
            if fallback != requested_backend and _is_backend_available(fallback):
                logger.warning(
                    "Video backend '%s' is not available (%s), falling back to '%s'. "
...
def get_frames_by_indices(
    ...
    if video_backend == "torchcodec":
        VideoDecoder = _lazy_import_torchcodec()
