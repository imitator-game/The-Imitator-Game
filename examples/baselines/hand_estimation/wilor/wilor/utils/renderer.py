...
class Renderer:
    ...
    def render_rgba_multiple(
            ...
            focal_length=None,
            is_right=None,
            origin_markers: Optional[List[bool]] = None,
            origin_marker_radius: float = 0.015,
            origin_marker_color: tuple = (1.0, 0.0, 0.0),
        ):
        """
        Render multiple hand meshes.
        
        Args:
            origin_markers: If provided, draw a sphere at origin (0,0,0) for hands where origin_markers[i] is True
            origin_marker_radius: Radius of the origin marker sphere
            origin_marker_color: Color of the origin marker sphere (RGB, 0-1)
        """
        ...
        # Support per-hand colors: if mesh_base_color is a list, use corresponding color for each hand
        if isinstance(mesh_base_color, (list, tuple)) and len(mesh_base_color) > 0 and isinstance(mesh_base_color[0], (list, tuple)):
            # mesh_base_color is a list of colors, one per hand
            mesh_list = [pyrender.Mesh.from_trimesh(self.vertices_to_trimesh(vvv, ttt.copy(), color, rot_axis, rot_angle, is_right=sss)) 
                        for vvv, ttt, sss, color in zip(vertices, cam_t, is_right, mesh_base_color)]
        else:
            # mesh_base_color is a single color, use it for all hands
        ...
        # Create origin marker spheres for specified hands
        sphere_meshes = []
        if origin_markers:
            for i, should_mark in enumerate(origin_markers):
                if should_mark and i < len(cam_t):
                    # Create sphere at origin (0,0,0) with cam_t translation
                    sphere = trimesh.creation.icosphere(subdivisions=2, radius=origin_marker_radius)
                    sphere_vertices = sphere.vertices + cam_t[i]
                    # Apply 180 degree rotation around X (same as hand mesh)
                    rot = trimesh.transformations.rotation_matrix(np.radians(180), [1, 0, 0])
                    sphere_tm = trimesh.Trimesh(vertices=sphere_vertices, faces=sphere.faces,
                                                vertex_colors=np.array([(*origin_marker_color, 1.0)] * len(sphere_vertices)))
                    sphere_tm.apply_transform(rot)
                    sphere_meshes.append(pyrender.Mesh.from_trimesh(sphere_tm))
        ...
        # Add origin marker spheres to scene
        for i, sphere_mesh in enumerate(sphere_meshes):
            scene.add(sphere_mesh, f'origin_marker_{i}')
