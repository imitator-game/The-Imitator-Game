...
VALID_VIDEO_CODECS = {"h264", "hevc", "libsvtav1", "libx265", "auto"} | set(HW_ENCODERS)
...
def _get_codec_options(
    ...
    if crf is not None:
        if vcodec in ("h264", "hevc", "libsvtav1", "libx265"):
            options["crf"] = str(crf)
...
class VideoDecoderCache:
    ...
    def __init__(self):
        ...
        self._lock = Lock()
        # torchcodec source support changed across versions. Probe once per process,
        # then reuse the working strategy to avoid repeated failed constructor calls.
        self._decoder_source_mode: str | None = None

    @staticmethod
    def _is_source_type_error(exc: Exception) -> bool:
        text = str(exc).lower()
        return (
            "unknown source type" in text
            or "supported types are" in text
            or "unsupported source" in text
        )

    def _create_decoder(self, video_path: str):
        if importlib.util.find_spec("torchcodec"):
        ...
        def _try_path_source():
            return VideoDecoder(video_path, seek_mode="approximate"), None

        def _try_fsspec_handle():
        ...
        constructors = {
            "path": _try_path_source,
            "fsspec_handle": _try_fsspec_handle,
        }

        preferred_modes = (
            [self._decoder_source_mode]
            if self._decoder_source_mode in constructors
            else ["path", "fsspec_handle"]
        )
        fallback_modes = [mode for mode in constructors if mode not in preferred_modes]

        for mode in preferred_modes + fallback_modes:
            tried_modes.append(mode)
            try:
                decoder, resource = constructors[mode]()
                self._decoder_source_mode = mode
                return decoder, resource
            except Exception as exc:  # nosec B110
                # Only fall back across source modes when the current torchcodec
                # explicitly rejects the source type. This avoids masking real
                # file/decoder errors and prevents local-path environments from
                # spuriously dropping into the legacy fsspec-handle path.
                if mode == "path" and not self._is_source_type_error(exc):
                    raise
                last_error = exc

        raise RuntimeError(
            "Failed to initialize torchcodec VideoDecoder for "
            f"{video_path!r} using modes {tried_modes}. Last error: {last_error}"
        ) from last_error
    ...
    def get_decoder(self, video_path: str):
        ...
        with self._lock:
            if video_path not in self._cache:
                decoder, resource = self._create_decoder(video_path)
                self._cache[video_path] = (decoder, resource)
    ...
    def clear(self):
        ...
        with self._lock:
            for _, resource in self._cache.values():
                if resource is not None:
                    resource.close()
            self._cache.clear()

    def invalidate(self, video_path: str) -> None:
        """Drop one cached decoder and close its associated resource."""
        video_path = str(video_path)

        with self._lock:
            decoder_and_resource = self._cache.pop(video_path, None)

        if decoder_and_resource is not None:
            _, resource = decoder_and_resource
            if resource is not None:
                resource.close()
...
def decode_video_frames_torchcodec(
    ...
    def _decode_with_decoder(decoder: Any) -> torch.Tensor:
        loaded_ts = []
        loaded_frames = []
    ...
    try:
        return _decode_with_decoder(decoder_cache.get_decoder(video_path))
    except FrameTimestampError:
        logging.warning(
            "torchcodec cached decoder seek mismatch for %s; retrying with a fresh decoder",
            video_path,
        )
        decoder_cache.invalidate(video_path)

        decoder, resource = decoder_cache._create_decoder(video_path)
        try:
            frames = _decode_with_decoder(decoder)
        finally:
            if resource is not None:
                resource.close()

        logging.info("torchcodec fresh-decoder retry succeeded for %s", video_path)
        return frames
...
def encode_video_frames(
    ...
    if vcodec == "libx265":
        # video_options["x265-params"] = "lossless=1"
        video_options["x265-params"] = "crf=18"
