...
def convert_obs(obs, concat_fn, transpose_fn, state_obs_extractor, depth=True, use_hand_camera=True):
    """
    Convert observation to Pi-compatible format with multi-camera support.

    Args:
        obs: Observation dictionary from ManiSkill
        concat_fn: Function to concatenate arrays
        transpose_fn: Function to transpose arrays
        state_obs_extractor: Function to extract state observations
        depth: Whether to include depth information
        use_hand_camera: Whether to use hand camera (in addition to base camera)

    Returns:
        Dictionary with separated camera views for Pi model
    """
    ...
    # Extract each camera separately
    new_img_dict = {}

    # Base camera
    if "base_camera" in img_dict:
        for key in ls:
            camera_key = f"base_camera_{key}"
            new_img_dict[camera_key] = transpose_fn(img_dict["base_camera"][key])

    # Hand camera (if available and requested)
    if use_hand_camera and "hand_camera" in img_dict:
        for key in ls:
            camera_key = f"hand_camera_{key}"
            new_img_dict[camera_key] = transpose_fn(img_dict["hand_camera"][key])

    # Convert depth to float16 if needed
    for key in new_img_dict:
        if "depth" in key and isinstance(new_img_dict[key], torch.Tensor):
            new_img_dict[key] = new_img_dict[key].to(torch.float16)

    # Unified version for state extraction
    ...
    # Add all camera views to output
    out_dict.update(new_img_dict)
