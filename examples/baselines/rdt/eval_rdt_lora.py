"""
RDT LoRA Model Evaluation Script - Simplified Version
Direct language usage without video conditioning
"""
...
from examples.baselines.rdt.train_rdt_lora import LoRARDTAgent, LoRAArgs
from examples.baselines.rdt.utils.make_env import make_eval_envs
from examples.baselines.rdt.utils.eval import evaluate
from examples.baselines.rdt.utils.precomputed_lang_loader import PrecomputedLangEmbedding
...
class StrictPrecomputedTextCache:
    """Adapter exposing get_text_embeddings without any online T5 fallback."""

    def __init__(self, cache, device: torch.device, max_length: int):
        self.cache = cache
        self.device = device
        self.max_length = max_length

    def get_text_embeddings(self, texts: list[str]) -> tuple[torch.Tensor, torch.Tensor]:
        if isinstance(self.cache, PrecomputedVLFeatures):
            cached = self.cache.get_batch([], [str(text) for text in texts])
            if cached is None or "lang_embeds" not in cached or "lang_mask" not in cached:
                known = getattr(self.cache, "_text_cache", None) or {}
                missing = [str(text) for text in texts if str(text) not in known]
                preview = ", ".join(repr(text[:80]) for text in missing[:3])
                raise KeyError(
                    "Missing language prompt(s) in precomputed language cache. "
                    f"First missing: {preview}"
                )
            return cached["lang_embeds"].to(self.device), cached["lang_mask"].to(self.device).bool()

        embeds, masks = self.cache.get_batch_embeddings(texts, self.max_length, return_mask=True)
        missing = []
        for text, embed, mask in zip(texts, embeds, masks):
            if not bool(mask.any()) and torch.allclose(embed, torch.zeros_like(embed)):
                missing.append(str(text))
        if missing:
            preview = ", ".join(repr(text[:80]) for text in missing[:3])
            raise KeyError(
                "Missing language prompt(s) in legacy precomputed language cache. "
                f"First missing: {preview}"
            )
        return embeds.to(self.device), masks.to(self.device).bool()
...
class EvalArgs:
    ...
    """Path to pretrained RDT model (if LoRA was trained on top of pretrained model)"""

    # Language embedding options
    use_precomputed_lang: bool = False
    """Use the legacy precomputed language embedding format"""
    precomputed_lang_dir: Optional[str] = None
    """Directory containing legacy precomputed language embeddings"""
    use_precomputed_vl_features: bool = True
    """Use language-only precomputed V/L cache when precomputed_vl_dir is set"""
    precomputed_vl_dir: Optional[str] = None
    """Directory containing language-only precomputed V/L features"""
    precomputed_vl_preload: bool = False
    """Preload precomputed V/L features into host memory"""
    precomputed_vl_shard_cache_size: int = 8
    """Number of precomputed V/L shards to keep cached"""
    expected_precomputed_vl_mode: str = "language_only"
    """Expected precomputed feature mode for eval"""
    allow_online_text_encoder: bool = False
    """Allow loading T5 at eval time when no precomputed language cache is available"""
...
if __name__ == "__main__":
    ...
    cli_precomputed_vl_dir = args.precomputed_vl_dir
    ...
    if args.load_model_config_from_ckpt and os.path.exists(args.checkpoint_path):
        ...
        if isinstance(ckpt_args, dict):
            model_keys = {
                ...
                "precomputed_vl_preload",
                "precomputed_vl_shard_cache_size",
                "expected_precomputed_vl_mode",
            }
            for key in model_keys:
                if key in ckpt_args:
                    setattr(args, key, ckpt_args[key])
            if cli_precomputed_vl_dir:
                args.precomputed_vl_dir = cli_precomputed_vl_dir
                args.use_precomputed_vl_features = True
                print(f"[ckpt] using CLI precomputed_vl_dir: {args.precomputed_vl_dir}")
            elif "precomputed_vl_dir" in ckpt_args:
                args.precomputed_vl_dir = ckpt_args["precomputed_vl_dir"]
            if getattr(args, "precomputed_vl_dir", None):
                args.use_precomputed_vl_features = True
    ...
    # Setup precomputed language embeddings. By default eval does not load T5;
    # prompts must be served from a language-only cache unless explicitly allowed.
    ...
    if args.use_precomputed_vl_features and args.precomputed_vl_dir:
        ...
        if args.expected_precomputed_vl_mode and actual_mode != args.expected_precomputed_vl_mode:
            raise ValueError(
                "Precomputed V/L mode mismatch for eval: "
                f"actual={actual_mode!r} expected={args.expected_precomputed_vl_mode!r} "
                f"dir={args.precomputed_vl_dir}"
            )
        precomputed_lang = StrictPrecomputedTextCache(precomputed_vl, device, args.max_lang_len)
        print(f"Precomputed language-only V/L stats: {precomputed_lang.get_stats()}")
    elif args.use_precomputed_lang and args.precomputed_lang_dir:
        legacy_cache = PrecomputedLangEmbedding(
            args.precomputed_lang_dir,
            device=device,
            embed_dim={
                "t5-v1_1-xxl": 4096,
                "t5-v1_1-base": 768,
        ...
        precomputed_lang = StrictPrecomputedTextCache(legacy_cache, device, args.max_lang_len)
        print(f"Legacy precomputed language stats: {precomputed_lang.get_stats()}")
    elif not args.use_dummy_language and not args.allow_online_text_encoder:
        raise ValueError(
            "Eval would need to load the text encoder, but allow_online_text_encoder=False. "
            "Pass --precomputed_vl_dir /path/to/language_only_cache (preferred), "
            "or explicitly pass --allow_online_text_encoder if you have enough VRAM."
        )
    ...
    print(f"RDT slot mapping: {args.rdt_slot_mapping}")
    print(f"Precomputed language cache: {precomputed_lang is not None}")
    print(f"Online text encoder allowed: {args.allow_online_text_encoder}")
    print(f"{'='*60}\n")

    # Create LoRA agent
    dummy_lora_args = LoRAArgs(
        env_id=args.env_id,
        ...
        rdt_slot_mapping=args.rdt_slot_mapping,
        ...
        use_precomputed_lang=(precomputed_lang is not None),
    ...
    agent = LoRARDTAgent(dummy_lora_args, envs, device, precomputed_lang)
    ...
    print(f"  Precomputed embeddings: {precomputed_lang is not None}")
    ...
    if args.save_results:
        ...
        results_with_metadata = {
            ...
            "config": {
                "model_type": "lora",
                "use_precomputed_lang": precomputed_lang is not None,
                "precomputed_vl_dir": args.precomputed_vl_dir,
                "lora_r": args.lora_r,
                "lora_alpha": args.lora_alpha,
                "eval_time": eval_time,
            }
