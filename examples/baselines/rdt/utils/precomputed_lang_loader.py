...
class PrecomputedLangEmbedding:
    ...
    def __init__(self, precomputed_dir: str, device: str = "cuda", embed_dim: int = 4096):
    ...
    def get_batch_embeddings(
        ...
        for text in texts:
            ...
            if embedding is None:
                ...
                embedding = torch.zeros(max_length, self.embed_dim, device=self.device)
...
class HybridLangEmbedding:
    ...
    def __init__(
        ...
        if t5_version not in lang_dim_map:
            raise ValueError(f"Unsupported t5_version: {t5_version}")
        self.embed_dim = lang_dim_map[t5_version]

        # Initialize precomputed embeddings if available
        self.precomputed = None
        if precomputed_dir and os.path.exists(precomputed_dir):
            try:
                self.precomputed = PrecomputedLangEmbedding(
                    precomputed_dir, device, embed_dim=self.embed_dim
                )
    ...
    def get_text_embeddings(self, texts: List[str]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            ...
            masks: [B, max_length]
        """
        def _pad_or_truncate(embedding: torch.Tensor, mask: torch.Tensor | None):
            seq_len = embedding.shape[0]
            if seq_len > self.max_length:
                ...
                embedding = torch.cat(
                    [embedding, torch.zeros(pad_len, embedding.shape[1], device=self.device)], dim=0
                )
                if mask is not None:
                    mask = torch.cat(
                        [mask, torch.zeros(pad_len, dtype=torch.bool, device=self.device)], dim=0
                    )
            return embedding, mask
            ...
            try:
                ...
                if missing_indices and self.fallback_embedder:
                    ...
                    for i, idx in enumerate(missing_indices):
                        fallback_emb = fallback_embeddings[i]
                        fallback_mask = fallback_masks[i]
                        fallback_emb, fallback_mask = _pad_or_truncate(fallback_emb, fallback_mask)
                        embeddings[idx] = fallback_emb
                        masks[idx] = fallback_mask
        ...
        if self.fallback_embedder:
            embeddings, masks = self.fallback_embedder.get_text_embeddings(texts)
            padded_embeddings = []
            padded_masks = []
            for emb, mask in zip(embeddings, masks, strict=True):
                emb, mask = _pad_or_truncate(emb, mask)
            ...
            B = len(texts)
            embeddings = torch.zeros(B, self.max_length, self.embed_dim, device=self.device)
