...
from examples.baselines.vqbet.vqbet.vqvae_utils import *
import einops
from examples.baselines.vqbet.vqbet.residual_vq import ResidualVQ
...
class VqVae(nn.Module):
    def __init__(
        ...
        if load_dir is not None:
            ...
            self.load_vqvae_state_dict(state_dict)
    ...
    def get_vqvae_state_dict(self):
        """
        Returns a custom checkpoint dict containing model weights + optimizer state.
        Use this when saving a standalone VqVae checkpoint to disk.
        """
        return {
    ...
    def load_vqvae_state_dict(self, state_dict):
        """
        Loads from a custom checkpoint dict produced by get_vqvae_state_dict().
        Use this when restoring a standalone VqVae checkpoint from disk.
        """
        ...
        self.vq_layer.load_state_dict(state_dict["vq_embedding"], strict=False)
        self.vq_layer.eval()