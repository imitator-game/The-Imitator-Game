...
class ParallelEvalXSkillScheduler:
    def __init__(self, args):
        ...
        self.log_dir = self.output_dir / f"scheduler_logs_xskill_{ts}"
    ...
    def build_workers(self, pending_envs: List[str], gpu_ids: List[int]) -> List[WorkerTask]:
        ...
        for worker_idx in range(num_workers):
            ...
            tmp = tempfile.NamedTemporaryFile(
                mode="w",
                suffix=".txt",
                prefix=f"eval_xskill_worker{worker_idx}_",
                dir=self.log_dir,
                delete=False,
                encoding="utf-8",
            )
    ...
    def _eval_entry_cmd_prefix(self) -> List[str]:
        # Prefer module mode so repo-relative imports are stable.
        # Use --eval-script only when you really want to run a file path directly.
    ...
    def run(self):
        self.log("=" * 80)
        self.log("PARALLEL EVAL SCHEDULER -- XSkill")
        self.log("=" * 80)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Parallel evaluation scheduler for XSkill",
    ...
    scheduler.add_argument("--max-retries", type=int, default=2)
    ...
    entry.add_argument(
        "--eval-module",
        type=str,
        default="examples.baselines.xskill.eval_xskill",
    ...
    eval_args = parser.add_argument_group("Forwarded to XSkill eval")
    eval_args.add_argument("--eval-config", type=str, required=True)
    eval_args.add_argument("--checkpoint", type=str, required=True)
    eval_args.add_argument("--hydra-config", type=str, required=True)
    ...
    eval_args.add_argument("--num-tasks", type=int, default=200)
    parser.add_argument("--temporal-agg", action="store_true", default=False)
    eval_args.add_argument("--eval_lr_mirror", default="false", choices=["auto", "true", "false"],
                        help="Override tabletop left-right mirror during eval.")
    eval_args.add_argument("--eval_lr_mirror_robot_pose", default="false", choices=["auto", "true", "false"],
    ...
    dtw.add_argument("--compute-dtw", dest="compute_dtw", action="store_true", default=True)
...
if __name__ == "__main__":
    ParallelEvalXSkillScheduler(parse_args()).run()
