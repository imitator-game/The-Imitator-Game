...
class Model(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.training_step_helper(robot_batch, batch_idx, 'robot')
        self.training_step_helper(human_batch, batch_idx, 'human')

    # @profile
    def training_step_helper(self, batch, batch_idx, video_type):
        e_opt, s_opt = self.optimizers()
        # eps_im, _, _ = batch  #(B,T,3,h,w)
        # batch_size = eps_im.shape[0]
        im_q, im_k = batch.im_q  # (B, N, slide+1, C, H, W) OLD NAMING RULES
        ...
        B, N = im_q.shape[:2]
        swav_batch_im_q = im_q.view(B * N, *im_q.shape[2:])
        swav_batch_im_k = im_k.view(B * N, *im_k.shape[2:])
        ...
        with torch.no_grad():
            wandb.log({
                f'{video_type}_encoder_loss':
                encoder_loss,
                f'{video_type}_repre_loss':
                rep_loss,
                f'{video_type}_cluster_loss':
                cluster_loss,
                'epoch':
                self.trainer.current_epoch,
                f'{video_type}_skill_prior_loss':
                skill_prior_loss,
                f'encoder_lr':
            ...
            print(f'step {self.trainer.global_step:07d} encoder_loss: {encoder_loss:.4f}, rep_loss: {rep_loss:.4f}, \
                cluster_loss: {cluster_loss:.4f}, skill_prior_loss: {skill_prior_loss:.4f}, \
                T: {self.T:.4f}')
    # @profile
    @torch.no_grad()
