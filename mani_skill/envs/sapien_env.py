...
class BaseEnv(gym.Env):
    ...
    def _setup_sensors(self, options: dict):
        ...
        for uid, sensor_config in self._sensor_configs.items():
            if uid in self._agent_sensor_configs and not isinstance(self.agent, MultiAgent):
    ...
    def reset(self, seed: Union[None, int, list[int]] = None, options: Union[None, dict] = None):
        ...
        if seed is not None or self._enhanced_determinism:
            ...
            self._initialize_episode(env_idx, options)
        # Set by _maybe_apply_lr_mirror when the scene mirror hook actually runs for this reset.
        self._lr_mirror_applied_this_reset = False
        self._maybe_apply_lr_mirror(env_idx, options)
    ...
    def _maybe_apply_lr_mirror(self, env_idx: torch.Tensor, options: dict):
        # Mirror only for dual tabletop tasks and only when L2/L3 are enabled.
        try:
            from mani_skill.envs.tasks.tabletop.utils.L0_L3_utils import (
                get_lr_mirror_euler_sxyz,
                get_lr_mirror_mode,
                is_lr_mirror_enabled,
                is_lr_mirror_robot_pose_enabled,
                mirror_quat,
                mirror_quat_euler_sxyz_z,
                mirror_quat_yaw,
                mirror_quat_xyz_flip_z,
                mirror_xyz,
            )
        except Exception:
            return
        if not is_lr_mirror_enabled():
            return
        if "mani_skill.envs.tasks.tabletop.dual_tasks" not in self.__class__.__module__:
            return
        self._lr_mirror_applied_this_reset = True
        mirror_robot_pose = is_lr_mirror_robot_pose_enabled()
        default_mode = get_lr_mirror_mode()
        target_actors = getattr(self, "_lr_mirror_target_actors", ())
        grasp_actors = getattr(self, "_lr_mirror_grasp_actors", ())
        target_articulations = getattr(self, "_lr_mirror_target_articulations", ())
        grasp_articulations = getattr(self, "_lr_mirror_grasp_articulations", ())
        rot_xyz_z_actors = getattr(self, "_lr_mirror_rot_xyz_z_actors", ())
        rot_xyz_z_articulations = getattr(self, "_lr_mirror_rot_xyz_z_articulations", ())
        euler_z_actors = getattr(self, "_lr_mirror_euler_z_actors", ())
        euler_z_articulations = getattr(self, "_lr_mirror_euler_z_articulations", ())

        def _collect_entity_ids(objs):
            ids = set()
            for obj in objs:
                if obj is None:
                    continue
                # Actor views created by Actor.merge expose underlying entities via _objs.
                if hasattr(obj, "_objs"):
                    ids.update(id(ent) for ent in obj._objs)
                ids.add(id(obj))
            return ids

        def _entity_in_ids(entity, ids):
            if id(entity) in ids:
                return True
            if hasattr(entity, "_objs"):
                return any(id(ent) in ids for ent in entity._objs)
            return False

        target_actor_ids = _collect_entity_ids(target_actors)
        grasp_actor_ids = _collect_entity_ids(grasp_actors)
        target_articulation_ids = _collect_entity_ids(target_articulations)
        grasp_articulation_ids = _collect_entity_ids(grasp_articulations)
        rot_xyz_z_actor_ids = _collect_entity_ids(rot_xyz_z_actors)
        rot_xyz_z_articulation_ids = _collect_entity_ids(rot_xyz_z_articulations)
        euler_z_actor_ids = _collect_entity_ids(euler_z_actors)
        euler_z_articulation_ids = _collect_entity_ids(euler_z_articulations)

        robot_articulation_ids = set()
        if getattr(self, "agent", None) is not None:
            if hasattr(self.agent, "agents"):
                for ag in self.agent.agents:
                    if hasattr(ag, "robot") and ag.robot is not None:
                        robot_articulation_ids.update(_collect_entity_ids((ag.robot,)))
            elif hasattr(self.agent, "robot") and self.agent.robot is not None:
                robot_articulation_ids.update(_collect_entity_ids((self.agent.robot,)))

        def _resolve_mode(entity, target_ids, grasp_ids, rot_xyz_z_ids, euler_z_ids):
            ent_id = id(entity)
            if ent_id in euler_z_ids:
                return "euler_z"
            if ent_id in rot_xyz_z_ids:
                return "pos_xyz_z"
            if ent_id in target_ids:
                return "pos_only"
            if ent_id in grasp_ids:
                return "pos_yaw"
            # Actor views or wrappers may expose underlying entities via _objs.
            if hasattr(entity, "_objs"):
                for ent in entity._objs:
                    ent_id = id(ent)
                    if ent_id in euler_z_ids:
                        return "euler_z"
                    if ent_id in rot_xyz_z_ids:
        ...
        for actor in self.scene.actors.values():
            ...
            if mode == "full":
                ...
                euler_override = get_lr_mirror_euler_sxyz(actor)
                if euler_override is None and hasattr(actor, "_objs"):
                    for ent in actor._objs:
                        euler_override = get_lr_mirror_euler_sxyz(ent)
                        if euler_override is not None:
                            break
                q = mirror_quat_euler_sxyz_z(pose.q, euler_override=euler_override)
            else:
                q = pose.q
            actor.set_pose(Pose.create_from_pq(p=p, q=q))

        for articulation in self.scene.articulations.values():
            if (not mirror_robot_pose) and _entity_in_ids(articulation, robot_articulation_ids):
                continue
            pose = articulation.pose
            mode = _resolve_mode(
                articulation,
                target_articulation_ids,
                grasp_articulation_ids,
                rot_xyz_z_articulation_ids,
                euler_z_articulation_ids,
            )
            p = mirror_xyz(pose.p)
            if mode == "full":
                q = mirror_quat(pose.q)
                ...
                euler_override = get_lr_mirror_euler_sxyz(articulation)
                if euler_override is None and hasattr(articulation, "_objs"):
                    for ent in articulation._objs:
                        euler_override = get_lr_mirror_euler_sxyz(ent)
                        if euler_override is not None:
                            break
                q = mirror_quat_euler_sxyz_z(pose.q, euler_override=euler_override)
                ...
                q = pose.q
            articulation.set_pose(Pose.create_from_pq(p=p, q=q))

        if hasattr(self, "_after_lr_mirror"):
            self._after_lr_mirror(env_idx, options)
