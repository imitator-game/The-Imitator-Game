...
from mani_skill.agents.robots import Fetch, Panda, Realman, RealmanMobileBase
...
class SceneManipulationEnv(BaseEnv):
    ...
    SUPPORTED_ROBOTS = ["panda", "fetch", "realman", "realman_mobile_base"]
    agent: Union[Panda, Fetch, Realman, RealmanMobileBase]

    def __init__(
        self,
        *args,
        robot_uids="realman_mobile_base",
    ...
    def _default_human_render_camera_configs(self):
        ...
        elif self.robot_uids in ["realman", "realman_mobile_base"]:
            room_camera_pose = sapien_utils.look_at([2.5, -2.5, 2], [0.0, 0.0, 0])
            if type(self.scene_builder) == ReplicaCADSceneBuilder:
                room_camera_pose = sapien_utils.look_at([-1.4, -2.6, 1.8], [-2.0, -2.6, 1.0])
            room_camera_config = CameraConfig(
                "render_camera",
                room_camera_pose,
                512,
