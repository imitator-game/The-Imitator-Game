...
@register_env("TwoRobotPickRemoteControl-v1", max_episode_steps=100)
class TwoRobotPickRemoteControlEnv(BaseEnv):
    """
    **Task Description:**
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.remotecontrol_mass = 0.5
        (
            self.remotecontrol_modelname,
            self.remotecontrol_model_id,
            self.remotecontrol_scale,
            self.remotecontrol_replace_scale,
        ) = apply_l2_robotwin_config(
            "079_remotecontrol",
            model_id=0,
            override_name="079_remotecontrol",
            override_id=1,
        )
        self.plasticbox_modelname, self.plasticbox_model_id = apply_l3_robotwin_model(
            "062_plasticbox",
            model_id=7,
            override_name="076_breadbasket",
            override_id=3,
        )
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            xyz[:, 1] = -0.1  # Y position - right side
            xyz[:, 2] = self.remotecontrol_z  # Z position (on table surface)
