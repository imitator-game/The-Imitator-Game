...
@register_env("TwoRobotPlaceChipsRack-v1", max_episode_steps=200)
class TwoRobotPlaceChipsRackEnv(BaseEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    def __init__(
        ...
        **kwargs
        ...
        (
            self.chipstub_modelname,
            self.chipstub_model_id,
            self.chipstub_scale,
            self.chipstub_replace_scale,
        ) = apply_l2_robotwin_config(
            "025_chips-tub",
            model_id=0,
            base_scale=(0.5, 0.5, 0.5),
            base_replace_scale=False,
            override_name="025_chips-tub",
            override_id=1,
        ...
        (
            self.coffee_box_modelname,
            self.coffee_box_model_id,
            self.coffee_box_scale,
            self.coffee_box_replace_scale,
        ) = apply_l3_robotwin_config(
            "113_coffee-box",
            model_id=0,
            override_name="113_coffee-box",
            override_id=2,
        )
        self.coffee_box_on_stand_offset = -0.01
        # Bottle (robotwin object, placed on display stand)
        (
            self.bottle_modelname,
            self.bottle_model_id,
            self.bottle_scale,
            self.bottle_replace_scale,
        ) = apply_l3_robotwin_config(
            "114_bottle",
            model_id=3,
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            chipstub_xyz = apply_l1_offset_xy(chipstub_xyz, offset=(-0.1, 0.1))
    ...
    def evaluate(self):
        ...
        chipstub_to_stand_xy = torch.linalg.norm(
            chipstub_pos[:, :2] - stand_pos[:, :2], dim=-1
        )
    ...
    def _get_obs_extra(self, info: Dict):
        ...
        if "state" in self.obs_mode:
            obs.update(
                ...
                chipstub_to_stand_pos=self.displaystand_bottom.pose.p - self.chipstub.pose.p,
            )
        return obs
