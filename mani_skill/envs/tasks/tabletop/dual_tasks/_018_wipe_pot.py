...
@register_env("TwoRobotWipePot-v1", max_episode_steps=100)
class TwoRobotWipePotEnv(BaseEnv):
    """
    Task: Left arm picks up sponge, wipes the surface of the pot repeatedly.

    Reward phases (from solve script):
      1. reach    — left TCP approaches sponge                    [0, 1]
      2. grasp    — left arm grasps sponge                        [0, 1]
      3. approach — move sponge to pot (gated by grasp)           [0, 1]
      4. wipe     — accumulated wipe steps + distance on pot      [0, 1]
    ...
    # Wipe success thresholds (from original evaluate)
    WIPE_TARGET_STEPS = 80
    WIPE_TARGET_DIST = 0.2
    ...
    def _load_scene(self, options: dict):
        ...
        sponge_scale = [0.4] if is_l2_enabled() else [0.8]
        ...
        partnet_model_id = apply_l3_partnet_ids(["100051"], override_ids=["100040"])[0]
        ...
        self.pot = partnet_builder.build(
            name=f'{partnet_category}-{partnet_model_id}', fix_root_link=True
        )
        ...
        for link in self.pot.links:
            ...
            if partnet_category.lower() in [
                'microwave', 'oven', 'dishwasher', 'refrigerator', 'cabinet',
                'storagefurniture',
            ]:
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # Persistent wipe tracking
            if not hasattr(self, "sponge_ever_wipe_pot"):
                self.sponge_ever_wipe_pot = torch.zeros(
                    self.num_envs, dtype=torch.bool, device=self.device
                )
            self.sponge_ever_wipe_pot[env_idx] = False

            if not hasattr(self, "sponge_last_pos"):
                self.sponge_last_pos = self.sponge.pose.p.clone()
            if not hasattr(self, "sponge_wipe_dist"):
                self.sponge_wipe_dist = torch.zeros(self.num_envs, device=self.device)
            if not hasattr(self, "sponge_wipe_steps"):
                self.sponge_wipe_steps = torch.zeros(
                    self.num_envs, dtype=torch.int32, device=self.device
                )
            self.sponge_last_pos[env_idx] = self.sponge.pose.p[env_idx].clone()
            self.sponge_wipe_dist[env_idx] = 0.0
            self.sponge_wipe_steps[env_idx] = 0
    ...
    def evaluate(self):
        is_grasped = self.left_agent.is_grasping(self.sponge)

        sponge_to_pot_dist = torch.linalg.norm(
            self.sponge.pose.p - self.pot.pose.p, axis=1
        )
        is_sponge_on_pot = sponge_to_pot_dist < self.goal_thresh
        is_contact = is_sponge_on_pot & is_grasped
        self.sponge_ever_wipe_pot = self.sponge_ever_wipe_pot | is_contact
        ...
        self.sponge_wipe_dist += step_dist * is_contact.float()
        self.sponge_wipe_steps += is_contact.int()
        self.sponge_last_pos = self.sponge.pose.p.clone()
        ...
        success = (self.sponge_wipe_steps >= self.WIPE_TARGET_STEPS) & (
            self.sponge_wipe_dist >= self.WIPE_TARGET_DIST
        )

        result = dict(
            is_grasped=is_grasped,
            is_sponge_on_pot=is_sponge_on_pot,
            sponge_to_pot_dist=sponge_to_pot_dist,
            sponge_wipe_dist=self.sponge_wipe_dist,
            sponge_wipe_steps=self.sponge_wipe_steps,
            sponge_ever_wipe_pot=self.sponge_ever_wipe_pot,
            success=success,
        )
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        pot_pos = self.pot.pose.p                    # (B, 3)
        is_grasped = info["is_grasped"]              # (B,) bool
        ever_on_pot = info["sponge_ever_wipe_pot"]   # (B,) bool
        success = info["success"]                    # (B,) bool

        ones = torch.ones(self.num_envs, device=self.device)

        # ── Phase 1: REACH — left TCP → sponge ──────────────────────
        # Completion: grasped ⇒ 1.0
        r_reach = reach_reward(tcp_pos, sponge_pos, scale=5.0)
        r_reach = torch.where(is_grasped | ever_on_pot | success, ones, r_reach)
        self.reward_tracker.update("reach", r_reach)

        # ── Phase 2: GRASP — left arm grasps sponge ─────────────────
        # Completion: ever_on_pot ⇒ must have grasped ⇒ 1.0
        r_grasp = grasp_reward(tcp_pos, sponge_pos, is_grasped, proximity_scale=5.0)
        r_grasp = torch.where(ever_on_pot | success, ones, r_grasp)
        self.reward_tracker.update("grasp", r_grasp)

        # ── Phase 3: APPROACH — move sponge to pot while grasped ─────
        # Completion: ever_on_pot ⇒ 1.0
        r_approach = transport_reward(sponge_pos, pot_pos, is_grasped, scale=5.0)
        ...
        r_wipe = wipe_progress_reward(
            steps=self.sponge_wipe_steps,
            target_steps=self.WIPE_TARGET_STEPS,
            dist=self.sponge_wipe_dist,
            target_dist=self.WIPE_TARGET_DIST,
        )
