...
REWARD_PHASES = [
    "reach_scanner", "grasp_scanner",
    "reach_pillbottle", "grasp_pillbottle",
    "scan",
]


@register_env("TwoRobotScanPillBottle-v1", max_episode_steps=200)
class TwoRobotScanPillBottleEnv(BaseEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            scanner_xyz[:, :2] += torch.rand((b, 2)) * 0.02
    ...
    def _after_reconfigure(self, options: dict):
        ...
        self.scanner_z = 0.02  # Default height if no collision mesh

        # Get z-offset for pill bottle to place it on table surface
        collision_mesh = self.pillbottle.get_first_collision_mesh()
        self.pillbottle_z = 0.0  # Default height if no collision mesh
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        # Phase 1: REACH_SCANNER - left TCP approaches scanner
        r_reach_scanner = reach_reward(left_tcp, scanner_pos, scale=5.0)
        r_reach_scanner = torch.where(is_scanner_grasped, ones, r_reach_scanner)
        self.reward_tracker.update("reach_scanner", r_reach_scanner)

        # Phase 2: GRASP_SCANNER - proximity / confirmed grasp
        ...
        # Phase 3: REACH_PILLBOTTEL - right TCP approaches pillbottle
        r_reach_pillbottle = reach_reward(right_tcp, pillbottle_pos, scale=5.0)
        r_reach_pillbottle = torch.where(is_pillbottle_grasped, ones, r_reach_pillbottle)
        self.reward_tracker.update("reach_pillbottle", r_reach_pillbottle)

        # Phase 4: GRASP_PILLBOTTEL - proximity / confirmed grasp
        r_grasp_pillbottle = grasp_reward(right_tcp, pillbottle_pos, is_pillbottle_grasped, proximity_scale=5.0)
        r_grasp_pillbottle = torch.where(is_pillbottle_grasped, ones, r_grasp_pillbottle)
        self.reward_tracker.update("grasp_pillbottle", r_grasp_pillbottle)

        # Phase 5: SCAN - scanner reaches pill bottle
        scanner_to_bottle_dist = torch.linalg.norm(scanner_pos - pillbottle_pos, dim=-1) - 0.1
