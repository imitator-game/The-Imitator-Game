...
from mani_skill.envs.tasks.tabletop.utils.L0_L3_utils import (
    ...
    is_l2_enabled,
    is_lr_mirror_robot_pose_enabled,
    mirror_pose,
    mirror_panda_qpos_y,
)
...
@register_env("TwoRobotFoldBox-v1", max_episode_steps=100)
class TwoRobotFoldBoxEnv(BaseEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.box_model_ids = apply_l2_partnet_ids(["100154"], override_ids=["100224"])
        # Articulation import scale (L2 can be slightly larger for the replacement box).
        self.box_builder_scale = 0.205
        self.l2_box_builder_scale = 0.26
        # Optional L2-only hinge angle override for the box articulation joint (rad).
        # Keep the same default as L0/L1 unless you manually change this value.
        self.l2_box_hinge_q = 2.3
        self.scanner_modelname, self.scanner_model_id = "024_scanner", 2
        self.thresh = -3.0
    ...
    def _load_scene(self, options: dict):
        ...
        # Load scanner
        self.scanner_pose = sapien.Pose(
            p=[0.1, -0.2, 0.0],
            q=euler2quat(np.pi / 2, 0, 0)
        )
        scanner_actor_obj = create_actor(
            scene=self.scene,
            pose=self.scanner_pose,
        ...
        self.scanner = scanner_actor_obj.actor

        self.waypoint1 = sapien.Pose([-0.0207413, -0.161137, 0.25], [-0.0705284, 0.919453, 0.386573, 0.013893])
        self.waypoint2 = sapien.Pose([-0.102653, -0.0574181, 0.27], [0.31947, 0.86108, 0.347494, 0.189018])
        self.waypoint3 = sapien.Pose([-0.160096, -0.00949945, 0.37], [0.325356, 0.866503, 0.341442, 0.163507])
    ...
    def _load_box(self, origin_base, joint_types, model_ids, link_ids):
        for i, model_id in enumerate(model_ids):
            # Build articulation
            box_scale = self.box_builder_scale
            if is_l2_enabled():
                box_scale = self.l2_box_builder_scale
            partnet_builder = articulations.get_articulation_builder(
                self.scene, f"partnet-mobility:{model_id}", mode="box", scale=box_scale
            )
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # For euler(0, 0, γ) objects:
            # - default mirror rule is γ -> -γ
            # - L2 uses custom rule requested:
            #   mirror on : (3π/4 + π/2) then negate
            #   mirror off: 3π/4 - π/2
            box_z_rot = np.pi * 3 / 4
            if is_l2_enabled():
                if is_lr_mirror_enabled():
                    ...
                    box_z_rot -= np.pi / 2
            elif is_lr_mirror_enabled():
                box_z_rot = -box_z_rot
            q = torch.tensor(euler2quat(0, 0, box_z_rot), device=self.device).repeat(b, 1)
            self.box.set_pose(Pose.create_from_pq(p=box_xyz, q=q))

            xyz = torch.tensor(self.scanner_pose.p).repeat(b, 1)
            xyz[:, :2] += torch.rand((b, 2)) * 0.02
            xyz[:, 2] = self.scanner_zs[env_idx] + xyz[:, 2]
            # For euler(π/2, 0, γ) objects, correct y-mirror is γ → π - γ
            scanner_z_rot = 0.0
            if is_lr_mirror_enabled():
                scanner_z_rot = np.pi - scanner_z_rot  # 0 → π
            qs = torch.tensor(euler2quat(np.pi / 2, 0, scanner_z_rot)).repeat(b, 1)
            self.scanner.set_pose(Pose.create_from_pq(xyz, qs))

            if is_l1_enabled():
                init_qpos = [-0.529, 0.882, 0.449, -1.478, 0.603, 1.081, -0.106, 0.03999997, 0.03999998]
            else:
                init_qpos = [-0.23649118, 1.3390672, 0.5435115, -0.66542065, 0.7061269, 0.9741016, 0.275781, 0.03999997, 0.03999998]
            if is_lr_mirror_enabled():
                init_qpos = mirror_panda_qpos_y(init_qpos)
                init_qpos[6] += np.pi / 2  # Extra π/2 to keep gripper perpendicular to mirrored box
            target_robot = self.left_agent.robot
            if (
                is_l2_enabled()
                and is_lr_mirror_enabled()
                and not is_lr_mirror_robot_pose_enabled()
            ):
                # Keep the mirrored working-arm posture on the visually active
                # right arm when scene mirror is enabled but robot root poses stay put.
                target_robot = self.right_agent.robot
            target_robot.set_qpos(init_qpos)
    ...
    def _after_reconfigure(self, options: dict):
        ...
        joints_all = getattr(self._boxs[0], "joints", None)
        joints = [
            joint for joint in joints_all
            if joint.type[0] in ("revolute", "revolute_unwrapped")
        ]
        target_joint = joints[1]
        qlimits = target_joint.limits
        qmin, qmax = qlimits[..., 0], qlimits[..., 1]
        self.target_qpos = qmin.clone()

        self.scanner_zs = []
        collision_mesh = self.scanner.get_first_collision_mesh()
        self.scanner_zs.append(-collision_mesh.bounding_box.bounds[0, 2])
        self.scanner_zs = common.to_tensor(self.scanner_zs, device=self.device)
    ...
    def evaluate(self):
        # Evaluate specific joint: success if the second joint qpos is below threshold and static.
        joints_all = None
        if hasattr(self, "_boxs") and self._boxs:
            joints_all = getattr(self._boxs[0], "joints", None)
        if joints_all is None:
            close_enough = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        ...
        target_joint = joints[1]
        qpos = target_joint.qpos.squeeze(-1)
        if is_l2_enabled():
            self.thresh = 0.5
        close_enough = qpos <= self.thresh
        link = target_joint.child_link if target_joint.child_link is not None else target_joint.parent_link
        link_is_static = (torch.linalg.norm(link.angular_velocity, axis=1) <= 1) & (
            torch.linalg.norm(link.linear_velocity, axis=1) <= 0.1
        )
        # print(f"target_joint qpos: {qpos}")
        partnet_link_pos = self.partnet_link_positions()
        is_robot_static = self.left_agent.is_static(0.2) and self.right_agent.is_static(0.2)
        is_grasped = self.left_agent.is_grasping(self.scanner)
        ...
        result = dict(
            ...
            is_robot_static=is_robot_static,
            is_grasped=is_grasped,
            success=torch.logical_and(close_enough, link_is_static),
        )
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        # Phase 1: CLOSE - lid closed
        joints_all = getattr(self._boxs[0], "joints", None)
        joints = [
            joint for joint in joints_all
            if joint.type[0] in ("revolute", "revolute_unwrapped")
        ]
        qpos = joints[1].qpos.squeeze(-1)
        r_close = exp_reward(torch.abs(qpos - self.target_qpos), scale=2.0)
        self.reward_tracker.update("close", r_close)
