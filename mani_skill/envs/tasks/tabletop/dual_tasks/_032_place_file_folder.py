...
REWARD_PHASES = [
    "reach_file", "grasp_file", "transport_file", "place_file",
    "open_folder",
]

@register_env("TwoRobotPlaceFileFolder-v1", max_episode_steps=100)
class TwoRobotPlaceFileFolderEnv(BaseEnv):
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            if (
                ...
                # Keep the mirrored opening-arm posture on the visually active
                # left arm when scene mirror is enabled but robot root poses stay put.
                target_robot = self.left_agent.robot
            target_robot.set_qpos(right_qpos)
    ...
    def evaluate(self):
        ...
        # Left arm grasps the file
        is_file_grasped = self.left_agent.is_grasping(self.file)
        # Check if folder is open (lid angle > threshold)
        folder_qpos = self.folder.get_qpos()  # (B, num_joints)
        lid_qpos = folder_qpos[:, 0] if folder_qpos.shape[1] > 0 else torch.zeros(self.num_envs, device=self.device)
        is_folder_open = lid_qpos > 0.8  # Threshold for "open"
        is_robot_static = self.left_agent.is_static(0.2) and self.right_agent.is_static(0.2)

        result = dict(
            obj_to_goal_pos=obj_to_goal_pos,
            is_obj_placed=is_obj_placed,
            is_file_grasped=is_file_grasped,
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        is_file_grasped = info["is_file_grasped"]
        is_folder_open = info["is_folder_open"]
        is_obj_placed = info["is_obj_placed"]
        lid_qpos = info["lid_qpos"]
        success = info["success"]

        ones = torch.ones(self.num_envs, device=self.device)

        # Left arm: REACH_FILE
        r_reach_file = reach_reward(left_tcp, file_pos, scale=5.0)
        r_reach_file = torch.where(is_file_grasped | is_obj_placed | success, ones, r_reach_file)
        self.reward_tracker.update("reach_file", r_reach_file)

        # Left arm: GRASP_FILE
        r_grasp_file = grasp_reward(left_tcp, file_pos, is_file_grasped, proximity_scale=5.0)
        r_grasp_file = torch.where(is_obj_placed | success, ones, r_grasp_file)
        self.reward_tracker.update("grasp_file", r_grasp_file)

        # Left arm: TRANSPORT_FILE
        r_transport_file = transport_reward(file_pos, folder_pos, is_file_grasped, scale=5.0)
        r_transport_file = torch.where(is_obj_placed | success, ones, r_transport_file)
        self.reward_tracker.update("transport_file", r_transport_file)

        # Left arm: PLACE_FILE (only when transport started and released)
        transport_peak = self.reward_tracker._peaks["transport_file"]
        r_place_file = is_obj_placed.float() * (transport_peak > 0).float() * (~is_file_grasped).float()
        r_place_file = torch.where(success, ones, r_place_file)
        self.reward_tracker.update("place_file", r_place_file)

        # Right arm: OPEN_FOLDER - reward based on lid opening angle
        # Target: lid should open to allow file placement
        r_open_folder = torch.clamp(lid_qpos / 0.8, 0.0, 1.0)  # Normalize lid angle to [0, 1]
        r_open_folder = torch.where(is_folder_open | success, ones, r_open_folder)
        self.reward_tracker.update("open_folder", r_open_folder)
