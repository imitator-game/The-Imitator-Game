...
@register_env("TwoRobotPourLiquidMug-v1", max_episode_steps=100)
class TwoRobotPourLiquidMugEnv(BaseEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Extra shared Y offset for liquid + rack, only applied in _initialize_episode
        # when L2 and LR mirror are enabled.
        self.l2_mirror_liquid_rack_y_offset = 0.08
        self.liquid_cup_modelname, self.liquid_cup_model_id = apply_l2_robotwin_model(
            "021_cup",
            model_id=5,
            override_name="021_cup",
            override_id=0,
        )
        self.target_cup_modelname, self.target_cup_model_id = apply_l3_robotwin_model(
            "021_cup",
            model_id=6,
            override_name="021_cup",
            override_id=10,
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            apply_l2_mirror_y_offset = is_l2_enabled() and is_lr_mirror_enabled()
            y_offset = float(self.l2_mirror_liquid_rack_y_offset) if apply_l2_mirror_y_offset else 0.0
