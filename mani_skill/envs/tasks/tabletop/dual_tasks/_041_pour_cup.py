...
@register_env("TwoRobotPourCup-v1", max_episode_steps=200)
class TwoRobotPourCupEnv(BaseEnv):
    """
    **Task Description:**
    ...
    goal_thresh = 0.3  # Threshold for checking if kettle is above cup
    return_thresh = 0.3  # Threshold for checking if kettle returned to original position
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.cup2_l2_scale_factor = 0.6
        # L2-only cup2 Y offset (meters). Set this value to tune cup2 Y position.
        self.cup2_l2_y_offset = 0.16
        (
            self.cup2_modelname,
            self.cup2_model_id,
            self.cup2_scale,
    ...
    def _load_scene(self, options: dict):
        ...
        cup1_pose = sapien.Pose(p=[0.0, 0.1, 0.0], q=euler2quat(np.pi / 2, 0, 0))
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            for object, init_pose in zip(self.objects, self._object_initial_poses):
                ...
                if is_l2_enabled() and object is self.cup2:
                    xyz[:, 1] += float(self.cup2_l2_y_offset)
    ...
    def evaluate(self):
        left_grasped = self.left_agent.is_grasping(self.cup2)
        right_grasped = self.right_agent.is_grasping(self.cup2)
        is_grasped = left_grasped | right_grasped

        cup1_center = self._get_actor_geom_center(self.cup1)
        cup2_center = self._get_actor_geom_center(self.cup2)
        cup2_to_cup1 = cup2_center - cup1_center
        horizontal_dist = torch.linalg.norm(cup2_to_cup1[..., :2], dim=-1)
        cup2_height_above_cup1 = cup2_to_cup1[..., 2]
        is_above_cup = torch.logical_and(
            horizontal_dist <= self.goal_thresh,
            cup2_height_above_cup1 > 0.01,
        )
    ...
    def compute_dense_reward(self, obs: Any, action, info: Dict):
        ...
        left_dist = torch.linalg.norm(cup2_center - left_tcp, dim=-1)
        right_dist = torch.linalg.norm(cup2_center - right_tcp, dim=-1)
