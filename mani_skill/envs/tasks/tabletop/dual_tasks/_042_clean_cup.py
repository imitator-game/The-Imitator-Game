...
@register_env("TwoRobotCleanCup-v1", max_episode_steps=100)
class TwoRobotCleanCupEnv(BaseEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.brush_modelname, self.brush_model_id = apply_l2_robotwin_model(
            "083_brush",
            model_id=0,
            override_name="083_brush",
            override_id=1,
        )
    ...
    def _init_clean_points(self, env_idx: torch.Tensor):
        ...
        point_a_pose = goal_pose * sapien.Pose([0.0, 0.0, -0.16], euler2quat(0, -np.pi / 2, 0))
        point_b_pose = goal_pose * sapien.Pose([0.0, 0.0, -0.25], euler2quat(0, -np.pi / 2, 0))
