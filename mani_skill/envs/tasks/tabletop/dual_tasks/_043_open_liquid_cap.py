...
@register_env("TwoRobotOpenLiquidCap-v1", max_episode_steps=100)
class TwoRobotOpenLiquidCapEnv(BaseEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # L2-only cap size tuning. Adjust this factor as needed.
        self.cap_l2_scale_factor = 0.6
        (
            self.woodenblock_modelname,
            self.woodenblock_model_id,
            self.cap_scale,
            self.cap_replace_scale,
        ) = apply_l2_robotwin_config(
            "086_woodenblock",
            model_id=1,
            base_scale=(0.7, 0.2, 0.7),
            base_replace_scale=False,
            override_name="086_woodenblock",
            override_id=0,
            override_scale=(
                0.7 * self.cap_l2_scale_factor,
                0.2 * self.cap_l2_scale_factor,
                0.7 * self.cap_l2_scale_factor,
            ),
            override_replace_scale=False,
        )
        self.cup_modelname, self.cup_model_id = apply_l2_robotwin_model(
            "021_cup",
            model_id=5,
    ...
    def _load_scene(self, options: dict):
        ...
        cap_actor_obj = create_actor(
            ...
            replace_scale=self.cap_replace_scale,
            scale=self.cap_scale,
        )
        self.cap = cap_actor_obj.actor
        self.cap.set_mass(0.5)
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            shared_xy_offset = apply_l1_offset_xy(shared_xy_offset, offset=(-0.1, 0.1))
