...
@register_env("TwoRobotFoldTowel-v1", max_episode_steps=100)
class TwoRobotFoldTowelEnv(BaseEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.cloth_model_ids = apply_l2_partnet_ids(
            self.cloth_model_ids,
            override_ids=["10243"],
        )
        # Allow a larger towel only for L2 override.
        self.cloth_scale = 0.15
        if is_l2_enabled():
            self.cloth_scale = 0.17
        # L2-only cloth link init angle ratio:
        # 0.0 -> qmin, 1.0 -> qmax.
        # Use 0.0 by default to make the L2 cloth flatter at reset.
        self.l2_cloth_link_angle_ratio = 1
    ...
    # Task 49 L2 only swaps the cloth asset; keep the original non-mirrored layout.
    def _maybe_apply_lr_mirror(self, env_idx: torch.Tensor, options: dict):
        if is_l2_enabled():
            self._lr_mirror_applied_this_reset = False
            return
        return super()._maybe_apply_lr_mirror(env_idx, options)
    ...
    def _load_cloth(self, origin_base, joint_types, model_ids, link_ids):
        for i, model_id in enumerate(model_ids):
            ...
            partnet_builder = articulations.get_articulation_builder(
                self.scene, f"partnet-mobility:{model_id}", mode="laptop", scale=self.cloth_scale
            )
            ...
            for i in range(len(self.partnet_links[0])):
                ...
                if is_l2_enabled():
                    ratio = self.l2_cloth_link_angle_ratio
                    qpos_min.append(qmin + (qmax - qmin) * ratio)
                else:
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            cloth_xyz = apply_l1_offset_xy(cloth_xyz, offset=(-0.1, -0.1))
            if is_l2_enabled():
                cloth_xyz[:, 0] -= 0.05
