...
@register_env("TwoRobotPickAppleBananaToBasketsL3-v1", max_episode_steps=200)
class TwoRobotPickAppleBananaToBasketsEnvL3(BaseEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    BASKET_MODEL_NAME = "062_plasticbox"

    apple_out_xy = (-0.25, -0.25)
    banana_out_xy = (-0.25, 0.25)
    basket_apple_xy = (0.1, -0.25)
    basket_banana_xy = (0.1, 0.25)
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.basket_model_id = get_model_id(self.basket_modelname, model_id=2)
        self.basket_scale_cfg = (self.basket_scale,) * 3
        self.basket_replace_scale = False
    ...
    def _after_reconfigure(self, options: dict):
        ...
        def _compute_xy_bounds(obj) -> Tuple[torch.Tensor, torch.Tensor]:
            bounds = world_aabb_from_local_mesh(obj, self.device)
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            basket_apple_xyz = torch.tensor(self.basket_apple_pose.p, device=self.device, dtype=torch.float32).repeat(b, 1)
            basket_apple_xyz[:, :2] += torch.rand((b, 2), device=self.device) * 0.02
            basket_apple_xyz[:, 2] = self._basket_apple_z[0]
            basket_apple_q = torch.tensor(self.basket_apple_pose.q, device=self.device, dtype=torch.float32).repeat(b, 1)
            self.basket_apple.set_pose(Pose.create_from_pq(p=basket_apple_xyz, q=basket_apple_q))

            basket_banana_xyz = torch.tensor(self.basket_banana_pose.p, device=self.device, dtype=torch.float32).repeat(b, 1)
            basket_banana_xyz[:, :2] += torch.rand((b, 2), device=self.device) * 0.02
            basket_banana_xyz[:, 2] = self._basket_banana_z[0]
            basket_banana_q = torch.tensor(self.basket_banana_pose.q, device=self.device, dtype=torch.float32).repeat(b, 1)
            ...
            apple_out_p = torch.tensor(self.apple_out_pose.p, device=self.device, dtype=torch.float32).repeat(b, 1)
            apple_out_p[:, :2] += torch.rand((b, 2), device=self.device) * 0.02
            apple_out_p[:, 2] = self._apple_out_z[0]
            apple_out_q = torch.tensor(self.apple_out_pose.q, device=self.device, dtype=torch.float32).repeat(b, 1)
            self.apple_out.set_pose(Pose.create_from_pq(p=apple_out_p, q=apple_out_q))

            banana_out_p = torch.tensor(self.banana_out_pose.p, device=self.device, dtype=torch.float32).repeat(b, 1)
            banana_out_p[:, :2] += torch.rand((b, 2), device=self.device) * 0.02
            banana_out_p[:, 2] = self._banana_out_z[0]
            banana_out_q = torch.tensor(self.banana_out_pose.q, device=self.device, dtype=torch.float32).repeat(b, 1)
    ...
    def evaluate(self):
        ...
        basket_apple_bounds = world_aabb_from_local_mesh(self.basket_apple, self.device)
        basket_banana_bounds = world_aabb_from_local_mesh(self.basket_banana, self.device)
        basket_apple_xy_min, basket_apple_xy_max = basket_apple_bounds[:, 0, :2], basket_apple_bounds[:, 1, :2]
        basket_banana_xy_min, basket_banana_xy_max = basket_banana_bounds[:, 0, :2], basket_banana_bounds[:, 1, :2]
        basket_apple_z_max = basket_apple_bounds[:, 1, 2]
        basket_banana_z_max = basket_banana_bounds[:, 1, 2]
        ...
        banana_in_basket = self._is_in_basket(
            banana_out_pos,
            basket_banana_xy_min,
            basket_banana_xy_max,
            basket_banana_z_max,
        )
