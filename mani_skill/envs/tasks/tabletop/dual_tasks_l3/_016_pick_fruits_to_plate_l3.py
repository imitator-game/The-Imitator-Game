...
REWARD_PHASES = ['reach_a', 'grasp_a', 'transport_a', 'place_a', 'reach_b', 'grasp_b', 'transport_b', 'place_b']
...
@register_env("TwoRobotPickFruitsToPlateL3-v1", max_episode_steps=200)
class TwoRobotPickFruitsToPlateEnvL3(BaseEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.fruit_a_modelname = "011_banana"
        self.fruit_a_l2_q_offset = euler2quat(np.pi / 2, 0.0, 0.0)
        self.fruit_b_modelname, self.fruit_b_model_id = (
            "103_fruit",
            6,
        )
    ...
    def _load_scene(self, options: dict):
        ...
        banana_builder = actors.get_actor_builder(self.scene, id=f"ycb:{self.fruit_a_modelname}")
        banana_builder._mass = 0.5
        banana_builder.initial_pose = self.fruit_a_pose
        self.fruit_a = banana_builder.build(name="fruit_a")
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            plate_left_xyz = apply_l1_offset_xy(plate_left_xyz, offset=(0.0, -0.1))
            ...
            plate_right_xyz = apply_l1_offset_xy(plate_right_xyz, offset=(0.0, 0.1))
    ...
    def _is_in_plate(self, obj_pos: torch.Tensor, plate_pos: torch.Tensor) -> torch.Tensor:
        xy_dist = torch.linalg.norm(obj_pos[..., :2] - plate_pos[..., :2], axis=1)
    ...
    def evaluate(self):
        ...
        fruit_a_pos = self._get_actor_geom_center(self.fruit_a).repeat(self.num_envs, 1)
        fruit_b_pos = self._get_actor_geom_center(self.fruit_b).repeat(self.num_envs, 1)
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        a_in = info["fruit_a_in_plate"]
        b_in = info["fruit_b_in_plate"]
        success = info["success"]
        ones = torch.ones(self.num_envs, device=self.device)

        r_reach_a = reach_reward(left_tcp, fruit_a_pos, scale=5.0)
        r_reach_a = torch.where(is_a_grasped | a_in | success, ones, r_reach_a)
        r_grasp_a = grasp_reward(left_tcp, fruit_a_pos, is_a_grasped, proximity_scale=5.0)
        r_grasp_a = torch.where(a_in | success, ones, r_grasp_a)
        r_transport_a = transport_reward(fruit_a_pos, plate_pos, is_a_grasped, scale=5.0)
        r_transport_a = torch.where(a_in | success, ones, r_transport_a)
        r_place_a = a_in.float()
        r_place_a = torch.where(success, ones, r_place_a)

        r_reach_b = reach_reward(right_tcp, fruit_b_pos, scale=5.0)
        r_reach_b = torch.where(is_b_grasped | b_in | success, ones, r_reach_b)
        r_grasp_b = grasp_reward(right_tcp, fruit_b_pos, is_b_grasped, proximity_scale=5.0)
        r_grasp_b = torch.where(b_in | success, ones, r_grasp_b)
        r_transport_b = transport_reward(fruit_b_pos, plate_pos, is_b_grasped, scale=5.0)
        r_transport_b = torch.where(b_in | success, ones, r_transport_b)
        r_place_b = b_in.float()
        r_place_b = torch.where(success, ones, r_place_b)

        for name, val in [("reach_a", r_reach_a), ("grasp_a", r_grasp_a), ("transport_a", r_transport_a),
                          ("place_a", r_place_a), ("reach_b", r_reach_b), ("grasp_b", r_grasp_b),
                          ("transport_b", r_transport_b), ("place_b", r_place_b)]:
