...
@register_env("TwoRobotPourKettleL3-v1", max_episode_steps=200)
class TwoRobotPourKettleEnvL3(BaseEnv):
    """
    **Task Description:**
    ...
    goal_thresh = 0.01  # Threshold for checking if kettle is above cup
    return_thresh = 0.08  # Threshold for checking if kettle returned to original position
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.kettle_modelname, self.kettle_model_id = (
            "001_bottle",
            20,
        )
        self.cup_modelname, self.cup_model_id = (
            "021_cup",
            0,
        )
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # Initialize Kettle - fixed position on the right
            xyz = torch.zeros((b, 3), device=self.device)
            xyz[:, :2] = torch.tensor(self.kettle_pose.p[:2])
            xyz[:, 2] = self.kettle_z  # Z position (on table surface)
            ...
            if is_lr_mirror_enabled():
                z_rotation = 0  # mirror spout direction: π → π - π = 0
            ...
            # Store initial kettle position for "return to origin" check
            self.kettle_initial_pos = xyz.clone()

            # Initialize Cup - fixed position on the left
            xyz = torch.zeros((b, 3), device=self.device)
            xyz[:, :2] = torch.tensor(self.cup_pose.p[:2])
            xyz[:, 2] = self.cup_z  # Z position (on table surface)
            xyz[:, :2] += torch.rand((b, 2)) * 0.02

            # Fixed upright orientation - no randomization
            base_quat = euler2quat(np.pi / 2, 0.0, 0.0)
            qs = torch.tensor([base_quat] * b, device=self.device, dtype=torch.float32)
            self.cup.set_pose(Pose.create_from_pq(xyz, qs))

    def _after_reconfigure(self, options: dict):
        # Get z-offset for kettle to place it on table surface
        collision_mesh = self.kettle.get_first_collision_mesh()
        self.kettle_z = 0.0

        # Get z-offset for cup to place it on table surface
    ...
    def evaluate(self):
        ...
        Success if kettle was EVER (grasped AND above cup AND tilted)
        """
        ...
        is_above_cup = torch.logical_and(
            horizontal_dist <= 0.2 + self.goal_thresh,
            kettle_height_above_cup > 0.02  # At least 10cm above cup
        )
        ...
        currently_pouring = torch.logical_and(
            torch.logical_and(is_grasped, is_above_cup),
            is_tilted,
        )
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        r_pour = above_reward(kettle_pos, cup_pos, min_height=0.02, is_grasping=is_grasped, h_scale=5.0)
