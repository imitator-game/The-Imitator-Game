...
REWARD_PHASES = ['reach_0','grasp_0','transport_0','place_0','reach_1','grasp_1','transport_1','place_1']


@register_env("TwoRobotPickFoodL3-v1", max_episode_steps=100)
class TwoRobotPickFoodEnvL3(BaseEnv):
    """
        **Task Description:**
        The goal is to sequentially pick up two fruits and place them into a bin.
        The fruits must be picked up one at a time, and each fruit must be
        successfully placed inside the bin before proceeding to the next.
        ...
        **Randomizations:**
        ...
        The position and orientation of the bin are randomized within the workspace.
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.tabletrashbin_modelname, self.tabletrashbin_model_id = (
            '063_tabletrashbin',
            6,
        )
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        fruit0_pos = self._fruits_ycb_objs[0].pose.p
        fruit1_pos = self._fruits_ycb_objs[1].pose.p
        bin_pos = self.bin.pose.p
        is_gl = info["is_grasped_left"]
        is_gr = info["is_grasped_right"]
        f0_in = info["is_fruit0_placed"]
        f1_in = info["is_fruit1_placed"]
        success = info["success"]
        ones = torch.ones(self.num_envs, device=self.device)

        r_reach_0 = reach_reward(left_tcp, fruit0_pos, scale=5.0)
        r_reach_0 = torch.where(is_gl | f0_in | success, ones, r_reach_0)
        r_grasp_0 = grasp_reward(left_tcp, fruit0_pos, is_gl, proximity_scale=5.0)
        r_grasp_0 = torch.where(f0_in | success, ones, r_grasp_0)
        r_transport_0 = transport_reward(fruit0_pos, bin_pos, is_gl, scale=5.0)
        r_transport_0 = torch.where(f0_in | success, ones, r_transport_0)
        r_place_0 = f0_in.float()
        r_place_0 = torch.where(success, ones, r_place_0)

        r_reach_1 = reach_reward(right_tcp, fruit1_pos, scale=5.0)
        r_reach_1 = torch.where(is_gr | f1_in | success, ones, r_reach_1)
        r_grasp_1 = grasp_reward(right_tcp, fruit1_pos, is_gr, proximity_scale=5.0)
        r_grasp_1 = torch.where(f1_in | success, ones, r_grasp_1)
        r_transport_1 = transport_reward(fruit1_pos, bin_pos, is_gr, scale=5.0)
        r_transport_1 = torch.where(f1_in | success, ones, r_transport_1)
        r_place_1 = f1_in.float()
        r_place_1 = torch.where(success, ones, r_place_1)

        for name, val in [("reach_0",r_reach_0),("grasp_0",r_grasp_0),("transport_0",r_transport_0),("place_0",r_place_0),("reach_1",r_reach_1),("grasp_1",r_grasp_1),("transport_1",r_transport_1),("place_1",r_place_1)]:
