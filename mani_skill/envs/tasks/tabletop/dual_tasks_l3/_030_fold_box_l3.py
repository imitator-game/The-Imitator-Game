...
@register_env("TwoRobotFoldBoxL3-v1", max_episode_steps=100)
class TwoRobotFoldBoxEnvL3(BaseEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.box_model_ids = ["100141"] #"100224"
        # Articulation import scale (L2 can be slightly larger for the replacement box).
        self.box_builder_scale = 0.11
        self.l3_box_builder_scale = 0.26
        self.l3_box_hinge_q = 0.85
    ...
    def _load_box(self, origin_base, joint_types, model_ids, link_ids):
        for i, model_id in enumerate(model_ids):
            # Build articulation
            box_scale = self.l3_box_builder_scale
            #if is_l3_enabled():
                # box_scale = self.l3_box_builder_scale
            ...
            qmin, qmax = target_qlimits[..., 0], target_qlimits[..., 1]
            self.target_qpos = qmin 
            pos = torch.tensor(self.l3_box_hinge_q, device=self.device).expand(qmin.shape[0], 1)
            self.partnet_link.joint.set_qpos(pos)
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            box_z_rot = -np.pi * 1 / 4
            # Apply L2-specific extra 90deg around world Z before mirror handling.
            ...
            init_qpos = [-0.284, 0.715, 0.43, -1.406, 0.541, 1.496, 0.075, 0.03999997, 0.03999998]
    ...
    def evaluate(self):
        ...
        if len(joints) < 2:
            close_enough = self.partnet_link.joint.qpos <= -1.5
            partnet_link_pos = self.partnet_link_positions()
            is_robot_static = self.left_agent.is_static(0.2) and self.right_agent.is_static(0.2)
            is_grasped = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
            return dict(
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        # Phase 1: FOLD - fold the box (lid close)
        r_fold = exp_reward(torch.abs(self.partnet_link.joint.qpos - self.target_qpos), scale=2.0)
        self.reward_tracker.update("fold", r_fold)
