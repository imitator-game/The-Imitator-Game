...
@register_env("TwoRobotPlaceBrushRestL3-v1", max_episode_steps=100)
class TwoRobotPlaceBrushRestEnvL3(BaseEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.brush_pen_modelname, self.brush_pen_model_id = "093_brush-pen", 5
            
        self.book_modelname, self.book_model_id = "043_book", 1

        self.rest_modelname, self.rest_model_id = "021_cup", 0
    ...
    def _load_scene(self, options: dict):
        ...
        if is_l2_enabled() and self.brush_pen_model_id == 2:
    ...
    def evaluate(self):
        ...
        is_in_the_cup = self.brush.pose.p[0][2] >= 0.15
        ...
        success = torch.logical_and(is_obj_placed, is_in_the_cup)
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        brush_pos = info["brush_pos"]
        rest_pos = info["rest_pos"]
        brush_pos = brush_pos + torch.tensor([0.08, 0, 0], device=self.device)

        is_grasped = info["is_grasped"]
        is_obj_placed = info["is_obj_placed"]
        is_in_the_cup = info["is_in_the_cup"]
        success = info["success"]

        ones = torch.ones(self.num_envs, device=self.device)

        # Use whichever arm is closer
        left_to_brush_dist = torch.linalg.norm(left_tcp - brush_pos, dim=-1)
        right_to_brush_dist = torch.linalg.norm(right_tcp - brush_pos, dim=-1)
        closer_tcp = torch.where(left_to_brush_dist < right_to_brush_dist, left_tcp, right_tcp)

        # Phase 1: REACH - approach brush (only before grasping)
        reach_gate = ~is_grasped
        r_reach = reach_reward(closer_tcp[:, :2], brush_pos[:, :2], scale=5.0) * reach_gate.float()
        r_reach = torch.where(is_grasped, ones, r_reach)
        self.reward_tracker.update("reach", r_reach)

        # Phase 2: GRASP - grasp brush (only after reaching, before placed)
        grasp_gate = is_grasped 
        r_grasp = is_grasped.float() * grasp_gate.float()
        self.reward_tracker.update("grasp", r_grasp)

        # Phase 3: TRANSPORT - bring brush to rest (only after grasping, before placed)
        transport_gate = is_grasped 
        r_transport = transport_reward(brush_pos[:, :2], rest_pos[:, :2], is_grasped, scale=5.0) * transport_gate.float()
        ...
        r_place = exp_reward(brush_to_rest_dist, scale=5.0) * (transport_peak > 0).float() * (~is_grasped).float()
