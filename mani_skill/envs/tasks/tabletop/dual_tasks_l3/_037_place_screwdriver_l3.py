...
REWARD_PHASES = [
    "reach_screwdriver", "grasp_screwdriver", "transport_screwdriver", "touch_screwdriver",
]


@register_env("TwoRobotPlaceScrewdriverL3-v1", max_episode_steps=100)
class TwoRobotPlaceScrewdriverEnvL3(BaseEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.screwdriver_modelname, self.screwdriver_model_id = "020_hammer", 0
    ...
    def _load_scene(self, options: dict):
        ...
        # Load screwdriver
        screwdriver_q = euler2quat(-np.pi/8, np.pi, -np.pi / 2)
        screwdriver_p = np.array([0.05, -0.35, 0.1], dtype=np.float32)
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        screwdriver_pos = info["screwdriver_pos"]
        ...
        left_to_screwdriver_dist = torch.linalg.norm(left_tcp - screwdriver_pos, dim=-1)
        right_to_screwdriver_dist = torch.linalg.norm(right_tcp - screwdriver_pos, dim=-1)
        closer_tcp = torch.where(left_to_screwdriver_dist < right_to_screwdriver_dist, left_tcp, right_tcp)

        # Phase 1: REACH - approach screwdriver (only before grasping)
        reach_gate = ~is_grasped
        r_reach = reach_reward(closer_tcp, screwdriver_pos, scale=5.0) * reach_gate.float()
        r_reach = torch.where(is_grasped, ones, r_reach)
        self.reward_tracker.update("reach_screwdriver", r_reach)

        # Phase 2: GRASP - grasp screwdriver (only after reaching, before touching cube)
        grasp_gate = is_grasped & ~screwdriver_ever_touched_cube
        r_grasp = is_grasped.float() * grasp_gate.float()
        self.reward_tracker.update("grasp_screwdriver", r_grasp)

        # Phase 3: TRANSPORT - bring screwdriver to cube (only after grasping, before touching)
        transport_gate = is_grasped & ~screwdriver_ever_touched_cube
        screwdriver_pos_for_transport = screwdriver_pos + torch.tensor([0, 0.08, 0], device=self.device)  # use top of screwdriver for transport reward
        r_transport = transport_reward(screwdriver_pos_for_transport[:, :2], cube_pos[:, :2], is_grasped, scale=5.0) * transport_gate.float()
        r_transport = torch.where(success, ones, r_transport)
        self.reward_tracker.update("transport_screwdriver", r_transport)

        # Phase 4: Touch - touch screwdriver to cube (only after grasping)
        r_touch = success.float() * is_grasped.float()
        self.reward_tracker.update("touch_screwdriver", r_touch)
