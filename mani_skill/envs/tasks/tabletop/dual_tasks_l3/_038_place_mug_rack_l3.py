...
@register_env("TwoRobotPlaceMugRackL3-v1", max_episode_steps=100)
class TwoRobotPlaceMugRackEnvL3(BaseEnv):
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            if is_lr_mirror_enabled():
                mug_z_rot = np.pi - mug_z_rot  # π/4 → 3π/4
            ...
            xyz[:, :2] += torch.rand((b, 2)) * 0.02
            xyz[:, 2] = self.rack_zs[env_idx] * 0.03
            # For euler(π/2, 0, γ) objects, y-mirror: γ → π - γ
            rack_z_rot = -np.pi * 3 / 4
            if is_lr_mirror_enabled():
                rack_z_rot = np.pi - rack_z_rot  # -3π/4 → 7π/4 (= -π/4)
                rack_z_rot += np.pi  # manual 180deg flip to match mirrored rack facing
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        mug_pos = info["mug_pos"]
        rack_pos = info["rack_pos"]
        ...
        left_to_mug_dist = torch.linalg.norm(left_tcp - mug_pos, dim=-1)
        right_to_mug_dist = torch.linalg.norm(right_tcp - mug_pos, dim=-1)
        closer_tcp = torch.where(left_to_mug_dist < right_to_mug_dist, left_tcp, right_tcp)

        # Phase 1: REACH - approach mug (only before grasping)
        reach_gate = ~is_grasped
        r_reach = reach_reward(closer_tcp, mug_pos, scale=5) * reach_gate.float()
        r_reach = torch.where(is_grasped, ones, r_reach)  # full reward if placed
        self.reward_tracker.update("reach", r_reach)

        # Phase 2: GRASP - grasp mug (only after reaching, before placed)
        grasp_gate = is_grasped 
        r_grasp = is_grasped.float() * grasp_gate.float()
        self.reward_tracker.update("grasp", r_grasp)

        # Phase 3: TRANSPORT - bring mug to rack (only after grasping, before placed)
        transport_gate = is_grasped 
        r_transport = transport_reward(mug_pos[:, :2], rack_pos[:, :2], is_grasped, scale=3) * transport_gate.float()
        r_transport = torch.where(success, ones, r_transport)  # full reward if placed
        ...
        mug_to_rack_dist = torch.linalg.norm(mug_pos[:, :2] - rack_pos[:, :2], dim=-1)
        r_place = exp_reward(mug_to_rack_dist, scale=3) * (transport_peak > 0).float() * (~is_grasped).float()
        r_place = torch.where(success, ones, r_place)  # full reward if placed
