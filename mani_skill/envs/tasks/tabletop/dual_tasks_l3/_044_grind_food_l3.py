...
@register_env("TwoRobotGrindFoodL3-v1", max_episode_steps=100)
class TwoRobotGrindFoodEnvL3(BaseEnv):

    SUPPORTED_ROBOTS = [("panda_wristcam", "panda_wristcam")]
    agent: MultiAgent[Tuple[PandaWristCam, PandaWristCam]]
    cube_half_size = 0.02
    goal_thresh = 0.3
    grind_xy_thresh = 0.12
    grind_motion_thresh = 0.005
    grind_steps_required = 3
    grind_motion_target = 0.05
    ...
    def _load_scene(self, options: dict):
        ...
        # Load pillbottle as the single grasp target (replacing grind + block stack)
    ...
    def evaluate(self):
        ...
        currently_grinding = is_grasped & is_obj_placed & (
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        r_grasp = grasp_reward(
            tcp_pos, grind_pos, self.left_agent.is_grasping(self.grind), proximity_scale=5.0
        )
