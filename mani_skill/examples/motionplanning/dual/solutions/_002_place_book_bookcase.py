...
from mani_skill.envs.tasks import TwoRobotPlaceBookBookcaseEnv
...
def solve(env: TwoRobotPlaceBookBookcaseEnv, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    ...
    grasp_pose = env.left_agent.build_grasp_pose(approaching, closing, env.book.pose.sp.p) * sapien.Pose([0, -0.02, 0])
    goal_pose = sapien.Pose(env.bookcase.pose.sp.p, grasp_pose.q) * sapien.Pose([0, 0, -0.05])
    reach_pose2 = grasp_pose * sapien.Pose([-0.1, 0, -0.22])
    reach_pose3 = goal_pose * sapien.Pose([-0.2, 0, -0.22])
    ...
    # Grasp
    # -------------------------------------------------------------------------- #
    grasp_target_pose = grasp_pose if is_l2_enabled() else grasp_pose * sapien.Pose([0, 0, -0.05])
    ...
    left_planner.move_to_pose_with_screw(goal_pose * sapien.Pose([0, 0, -0.12]), other_gripper_state=right_planner.gripper_state)
