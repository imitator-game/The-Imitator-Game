...
from mani_skill.envs.tasks import TwoRobotPlaceMagazineFolderEnv
from mani_skill.examples.motionplanning.panda.motionplanner import \
    PandaArmMotionPlanningSolver
from mani_skill.envs.tasks.tabletop.utils.L0_L3_utils import mirror_offset_pose

def solve(env: TwoRobotPlaceMagazineFolderEnv, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    ...
    reach_pose1 = base_magazine * mirror_offset_pose(
        sapien.Pose(p=[0.05, 0.1, -0.15], q=euler2quat(0, 0, 0))
    )
    grasp_pose = base_magazine * mirror_offset_pose(
