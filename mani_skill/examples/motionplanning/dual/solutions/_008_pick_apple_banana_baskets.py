...
from mani_skill.envs.tasks import TwoRobotPickAppleBananaToBasketsEnv
...
def _pick_and_place(
    env: TwoRobotPickAppleBananaToBasketsEnv,
    ...
    if init_q is not None:
        grasp_pose = sapien.Pose(p=grasp_pose.p, q=init_q)
    pre_grasp_pose = grasp_pose * sapien.Pose([0, 0, -0.08])

    lift_pose = grasp_pose * sapien.Pose([0, 0, -0.12])
    place_pose = sapien.Pose(p=place_center_xyz + np.array([0.03, 0.03, 0.12]), q=grasp_pose.q)
...
def solve(env: TwoRobotPickAppleBananaToBasketsEnv, seed=None, debug: bool = False, vis: bool = False):
    env.reset(seed=seed)
    ...
    left_fixed_q = np.array(left_init_pose.q[0]) if is_l2_enabled() else None
    right_fixed_q = np.array(right_init_pose.q[0]) if is_l2_enabled() else None

    apple_basket_center = env.basket_apple.pose.sp.p
    banana_basket_center = env.basket_banana.pose.sp.p
    ...
    _pick_and_place(
        ...
        agent_id=0,
        obj=env.apple_out,
        place_center_xyz=apple_basket_center,
        init_q=left_fixed_q,
    )
    ...
    _pick_and_place(
        ...
        agent_id=1,
        obj=env.banana_out,
        place_center_xyz=banana_basket_center,
        init_q=right_fixed_q,
    )
