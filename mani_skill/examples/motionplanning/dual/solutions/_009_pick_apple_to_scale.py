...
from mani_skill.envs.tasks import TwoRobotPickAppleToScaleEnv
...
def _pick_and_place(
    env: TwoRobotPickAppleToScaleEnv,
    ...
    above_scale = sapien.Pose(p=place_center_xyz + np.array([0.0, 0.0, 0.20]), q=grasp_pose.q)
    place_pose = sapien.Pose(p=place_center_xyz + np.array([0.03, 0.0, 0.12]), q=grasp_pose.q)
...
def solve(env: TwoRobotPickAppleToScaleEnv, seed=None, debug: bool = False, vis: bool = False):
    env.reset(seed=seed)
    ...
    _pick_and_place(env, left_planner, right_planner, agent_id=0, obj=env.apple, place_center_xyz=scale_center_xyz, vis=vis)
