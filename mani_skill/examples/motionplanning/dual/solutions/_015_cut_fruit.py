...
from mani_skill.envs.tasks import TwoRobotCutFruitEnv
...
def solve(env: TwoRobotCutFruitEnv, seed=None, debug=False, vis=False):
    options = {
    "reconfigure": True
    }
    ...
    l2_only = is_l2_enabled() and not is_l3_enabled()

    knife_target = getattr(env, "knife_grasp_target", env.knife)
    obb = get_actor_obb(knife_target)
    ...
    if lr_mirror and l2_only:
        closing = -closing
    left_init_pose = env.left_agent.tcp.pose
    right_init_pose = env.right_agent.tcp.pose
    grasp_pose = env.left_agent.build_grasp_pose(approaching, closing, knife_target.pose.sp.p)
    if is_l2_enabled():
        grasp_pose = grasp_pose * sapien.Pose([0, 0.025, 0.0])
    goal_pose = sapien.Pose(env.carrot.pose.sp.p, grasp_pose.q) * mirror_offset_pose(
        sapien.Pose([0, 0, -0.05])
    )
    reach_pose2 = grasp_pose * mirror_offset_pose(sapien.Pose([-0.1, 0, -0.2]))
    grasp_entry_offset = mirror_offset_pose(
        sapien.Pose(p=[-0.13, 0, 0], q=euler2quat(np.pi * 3 / 8, 0, 0))
    )

    # -------------------------------------------------------------------------- #
    # Reach
    # -------------------------------------------------------------------------- #
    reach_pose1 = grasp_pose * mirror_offset_pose(sapien.Pose([-0.13, 0.1, -0.15]))
    ...
    reach_pose3 = goal_pose * mirror_offset_pose(sapien.Pose([-0.15, 0, -0.2]))
    left_planner.move_to_pose_with_screw(reach_pose3, other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(
        goal_pose * mirror_offset_pose(sapien.Pose([-0.15, 0, -0.05])),
        other_gripper_state=right_planner.gripper_state,
    )
