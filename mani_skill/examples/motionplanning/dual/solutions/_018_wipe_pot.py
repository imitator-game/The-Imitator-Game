...
from mani_skill.envs.tasks import TwoRobotWipePotEnv
from mani_skill.examples.motionplanning.panda.motionplanner import \
    PandaArmMotionPlanningSolver
from mani_skill.examples.motionplanning.panda.utils import (
    compute_grasp_info_by_obb, get_actor_obb)
from mani_skill.envs.tasks.tabletop.utils.L0_L3_utils import is_l2_enabled, is_lr_mirror_enabled, is_l3_enabled

def solve(env: TwoRobotWipePotEnv, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    ...
    # -----------------------------
    obb = get_actor_obb(env.sponge, to_world_frame=True, vis=False)
    ...
    point_a_offset_x = -0.08
    point_a_offset_y = -0.1
    point_b_offset_x = 0.08
    point_b_offset_y = -0.1
    if is_l2_enabled():
        point_a_offset_y -= 0.0
        point_b_offset_y -= 0.0
        point_a_offset_x += 0.05
    if is_lr_mirror_enabled():
        point_a_offset_y = -point_a_offset_y
        point_b_offset_y = -point_b_offset_y
    if is_l3_enabled():
        point_a_offset_y += 0.03
        point_b_offset_y += 0.03
    wipe_pose_1 = sapien.Pose(pot_pos + np.array([point_a_offset_x, point_a_offset_y, 0.03]), goal_pose.q)
    wipe_pose_2 = sapien.Pose(pot_pos + np.array([point_b_offset_x, point_b_offset_y, 0.03]), goal_pose.q)
    ...
    # -----------------------------
    # 4) Back to initial placement region, put down sponge
    # -----------------------------
