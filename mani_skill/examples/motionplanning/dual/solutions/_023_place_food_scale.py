...
def solve(env: TwoRobotPlaceFoodScaleEnv, seed=None, debug=False, vis=False):
    ...
    grasp_info = compute_grasp_info_by_obb(
        ...
        target_closing=None if is_l2_enabled() else target_closing,
        depth=FINGER_LENGTH,
    )
    closing, center = grasp_info["closing"], grasp_info["center"]
    left_init_pose = env.left_agent.tcp.pose
    right_init_pose = env.right_agent.tcp.pose
    grasp_pose = env.left_agent.build_grasp_pose(approaching, closing, env.food.pose.sp.p) * sapien.Pose([0, 0, -0.05])
    if is_l2_enabled():
        grasp_pose = grasp_pose * sapien.Pose([0, 0.0, 0.02])
    goal_pose = sapien.Pose(env.electronicscale.pose.sp.p, grasp_pose.q) * sapien.Pose([0, 0, -0.05])
    if is_l2_enabled():
        goal_pose = goal_pose * sapien.Pose([-0.05, -0.05, 0.0])
    ...
    reach_pose3 = goal_pose * sapien.Pose([0.06, -0.03, -0.2])
    left_planner.move_to_pose_with_screw(reach_pose3, other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(goal_pose * sapien.Pose([0.06, -0.03, -0.1]), other_gripper_state=right_planner.gripper_state)
