...
from mani_skill.envs.tasks import TwoRobotPlaceBurgerTrayEnv
from mani_skill.examples.motionplanning.panda.motionplanner import \
    PandaArmMotionPlanningSolver
from mani_skill.examples.motionplanning.panda.utils import (
    compute_grasp_info_by_obb, get_actor_obb)

def solve(env: TwoRobotPlaceBurgerTrayEnv, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    ...
    grasp_pose = env.left_agent.build_grasp_pose(approaching, closing, env.burger.pose.sp.p)
    goal_pose = sapien.Pose(env.tray.pose.sp.p, grasp_pose.q) * sapien.Pose([0, 0, -0.05])
