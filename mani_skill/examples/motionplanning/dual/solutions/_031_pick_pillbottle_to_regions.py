...
def _pick_and_place(
    ...
    above_place_pose = sapien.Pose(p=place_pose.p + np.array([0.0, 0.0, 0.08]), q=grasp_pose.q)
    end_pose = sapien.Pose( p=place_pose.p + np.array([0.0, 0.0, 0.12]), q=place_pose.q)
...
def solve(env: TwoRobotPickPillToRegionsEnv, seed=None, debug: bool = False, vis: bool = False):
    ...
    region_a_center = env.region_a.pose.sp.p
    region_b_center = env.region_b.pose.sp.p
    ...
    _pick_and_place(env, left_planner, right_planner, agent_id=0, obj=env.pill_a, place_center_xyz=region_a_center)
