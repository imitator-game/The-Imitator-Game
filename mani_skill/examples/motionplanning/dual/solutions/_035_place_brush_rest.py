...
from mani_skill.envs.tasks import TwoRobotPlaceBrushRestEnv
...
def solve(env: TwoRobotPlaceBrushRestEnv, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    ...
    if is_l2_enabled():
        grasp_pose = grasp_pose * sapien.Pose([0, 0, 0], euler2quat(0, 0, np.pi))
    goal_pose = sapien.Pose(env.rest.pose.sp.p, grasp_pose.q)
    reach_pose2 = grasp_pose * mirror_offset_pose(sapien.Pose([-0.03, 0.0, -0.2]))

    # -------------------------------------------------------------------------- #
    # Reach
    # -------------------------------------------------------------------------- #
    reach_pose1 = grasp_pose * mirror_offset_pose(sapien.Pose([-0.03, 0, -0.15]))
    left_planner.move_to_pose_with_screw(reach_pose1, other_gripper_state=right_planner.gripper_state)

    # -------------------------------------------------------------------------- #
    # Grasp
    # -------------------------------------------------------------------------- #
    left_planner.move_to_pose_with_screw(grasp_pose * mirror_offset_pose(sapien.Pose([-0.03, 0.0, 0])), other_gripper_state=right_planner.gripper_state)
    ...
    reach_pose3 = goal_pose * mirror_offset_pose(sapien.Pose([0.02, -0.05, -0.2], euler2quat(0, 0, np.pi / 2)))
    left_planner.move_to_pose_with_screw(reach_pose3, other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(goal_pose * mirror_offset_pose(sapien.Pose([0.02, -0.075, -0.05], euler2quat(0, 0, np.pi / 2))), other_gripper_state=right_planner.gripper_state)
