...
from mani_skill.envs.tasks import TwoRobotPourCupEnv
...
def solve(env: TwoRobotPourCupEnv, seed=None, debug=False, vis=False):
    """
    Phases:
    1. Grasp kettle
    2. Lift kettle
    ...
    # Store initial cup2 position for return phase
    cup2_initial_pos = env.cup2.pose.sp.p.copy()
    ...
    # ])
    approaching = np.array([0, np.sin(np.pi/3), -np.cos(np.pi/3)])
    ...
    # custom_position = env.cup2.pose.sp.p + custom_offset
    # grasp_pose = sapien.Pose(p=custom_position, q=grasp_pose_base.q)
    grasp_pose = grasp_pose_base
    grasp_z_offset = 0.04  # try 2cm first, adjustable between 0.01 and 0.04
    grasp_pose = sapien.Pose(
        p=grasp_pose.p + np.array([0.0, 0.0, grasp_z_offset], dtype=np.float32),
        q=grasp_pose.q,
    )
    if is_lr_mirror_enabled():
        # Scene object positions are already mirrored in env. Mirror only the gripper orientation here.
        mirrored_grasp_pose = mirror_pose(grasp_pose, mode="full")
        grasp_pose = sapien.Pose(grasp_pose.p, mirrored_grasp_pose.q)

    # -------------------------------------------------------------------------- #
    # Phase 1: Reach above cup2
    # -------------------------------------------------------------------------- #
    # reach_pose = grasp_pose * sapien.Pose([0, 0, -0.15])
    reach_pose = sapien.Pose(
        p=grasp_pose.p + mirror_xyz(np.array([0, -0.1, 0.1], dtype=np.float32)),
        q=grasp_pose.q,
    )
    ...
    # -------------------------------------------------------------------------- #
    # Phase 2: Descend and grasp cup2
    # -------------------------------------------------------------------------- #
    # L2 only: add a local pre-grasp retreat pose (10 cm backward in TCP local z)
    # to avoid jumping directly from reach_pose to grasp_pose.
    # if is_l2_enabled():
    #     pre_grasp_pose_l2 = sapien.Pose([0, 0.05, 0.1]) * grasp_pose
    #     left_planner.move_to_pose_with_screw(
    #         pre_grasp_pose_l2,
    #         other_gripper_state=right_planner.gripper_state
    #     )
    ...
    lift_pose = sapien.Pose(
        p=grasp_pose.p + mirror_xyz(np.array([0, 0, 0.10], dtype=np.float32)),
        q=grasp_pose.q,
    )
    left_planner.move_to_pose_with_screw(
        lift_pose,
    ...
    above_cup1_pose_base = sapien.Pose(
        env.cup1.pose.sp.p + mirror_xyz(np.array([0, -0.05, 0.25], dtype=np.float32)),  # 25cm above cup
        grasp_pose.q
    )
    # Add backward offset in gripper local frame (X-axis is approaching direction, -X is backward)
    backward_offset = sapien.Pose([0, 0, 0])  # 0cm backward in gripper frame
    above_cup1_pose = above_cup1_pose_base * mirror_offset_pose(backward_offset)
    ...
    pour_rotation = euler2quat(0, -np.pi/6, 0)
    ...
    pour_pose = sapien.Pose(
        above_cup1_pose.p,  # Same position as Phase 4
    ...
    return_above_pose = sapien.Pose(
        cup2_initial_pos + np.array([0, 0, 0.2]), 
        grasp_pose.q
    )
    left_planner.move_to_pose_with_screw(
        return_above_pose,
    ...
    place_pose = sapien.Pose(
        cup2_initial_pos + mirror_xyz(np.array([0, -0.1, 0.1], dtype=np.float32)),
        grasp_pose.q
    )
    left_planner.move_to_pose_with_screw(
        place_pose,
