...
def solve(env: TwoRobotPourLiquidCupEnv, seed=None, debug=False, vis=False):
    ...
    # Only when L2 + mirror is active, apply an extra world-Y offset to grasp pose.
    l2_mirror_grasp_y_offset = 0.03  # Tune this value (meters), e.g. 0.02 or -0.02
    if is_l2_enabled() and is_lr_mirror_enabled():
        grasp_pose = sapien.Pose(
            p=grasp_pose.p + np.array([0.0, l2_mirror_grasp_y_offset, 0.0], dtype=np.float32),
            q=grasp_pose.q,
        )
    goal_pose = sapien.Pose(env.cup.pose.sp.p, grasp_pose.q) * sapien.Pose([0, 0, -0.05])
    reach_pose2 = grasp_pose * sapien.Pose([-0.05, 0.02, -0.2])

    # -------------------------------------------------------------------------- #
    # Reach
    # -------------------------------------------------------------------------- #
    reach_pose1 = grasp_pose * sapien.Pose([-0.05, 0.02, -0.15])
    ...
    reach_pose3 = goal_pose * sapien.Pose([0.0, 0, -0.25], euler2quat(0, -np.pi / 2, 0))
    left_planner.move_to_pose_with_screw(reach_pose3, other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(goal_pose * sapien.Pose([0.0, 0, -0.16], euler2quat(0, -np.pi / 2, 0)), other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(reach_pose3, other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(goal_pose * sapien.Pose([0.0, 0, -0.16], euler2quat(0, -np.pi / 2, 0)), other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(reach_pose3, other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(goal_pose * sapien.Pose([0.0, 0, -0.16], euler2quat(0, -np.pi / 2, 0)), other_gripper_state=right_planner.gripper_state)
