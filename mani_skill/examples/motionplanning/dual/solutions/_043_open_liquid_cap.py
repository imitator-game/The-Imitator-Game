...
from mani_skill.envs.tasks import TwoRobotOpenLiquidCapEnv
from mani_skill.examples.motionplanning.panda.motionplanner import \
    PandaArmMotionPlanningSolver
from mani_skill.examples.motionplanning.panda.utils import (
    compute_grasp_info_by_obb, get_actor_obb)

def solve(env: TwoRobotOpenLiquidCapEnv, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    ...
    grasp_pose = env.left_agent.build_grasp_pose(approaching, closing, env.cap.pose.sp.p)
    goal_pose = sapien.Pose(env.plate.pose.sp.p, grasp_pose.q) * sapien.Pose([0, 0, -0.05])
    reach_pose2 = grasp_pose * sapien.Pose([0, 0, -0.1])
    ...
    reach_pose3 = goal_pose * sapien.Pose([0.0, 0., -0.3])
    left_planner.move_to_pose_with_screw(reach_pose3, other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(goal_pose * sapien.Pose([0., 0., -0.0]), other_gripper_state=right_planner.gripper_state)
