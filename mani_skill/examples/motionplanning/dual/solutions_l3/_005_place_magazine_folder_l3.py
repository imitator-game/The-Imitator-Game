...
from mani_skill.envs.tasks import TwoRobotPlaceMagazineFolderEnvL3
from mani_skill.examples.motionplanning.panda.motionplanner import \
    PandaArmMotionPlanningSolver
from mani_skill.examples.motionplanning.panda.utils import (
    compute_grasp_info_by_obb, get_actor_obb)

def solve(env: TwoRobotPlaceMagazineFolderEnvL3, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    ...
    base_magazine = sapien.Pose(env.magazine.pose.sp.p, np.array(left_init_pose.q[0]))
    base_folder = sapien.Pose(env.folder.pose.sp.p, np.array(left_init_pose.q[0]))

    reach_pose1 = base_magazine * (
        sapien.Pose(p=[0.05, 0.1, -0.15], q=euler2quat(0, 0, 0))
    )
    grasp_pose = base_magazine * (
        sapien.Pose(p=[0.0, 0.02, 0.0], q=euler2quat(np.pi / 2, 0, 0))
    )
    reach_pose2 = grasp_pose * (sapien.Pose([0, -0.15, 0.]))
    reach_pose3 = base_folder * (
        sapien.Pose(p=[0, 0.04, -0.2], q=euler2quat(np.pi / 2, 0, 0))
    )
    goal_pose = base_folder * (
        sapien.Pose(p=[0, 0.04, -0.1], q=euler2quat(np.pi / 6, 0, 0))
    )
    reach_pose4 = base_folder * (
        sapien.Pose(p=[0, 0.04, -0.2], q=euler2quat(np.pi / 8, 0, 0))
    )
