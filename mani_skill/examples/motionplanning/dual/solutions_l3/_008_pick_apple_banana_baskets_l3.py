...
from mani_skill.envs.tasks import TwoRobotPickAppleBananaToBasketsEnvL3
...
def _pick_and_place(
    env: TwoRobotPickAppleBananaToBasketsEnvL3,
    ...
    place_pose = sapien.Pose(p=place_center_xyz + np.array([0.0, 0.0, 0.2]), q=grasp_pose.q)
...
def solve(env: TwoRobotPickAppleBananaToBasketsEnvL3, seed=None, debug: bool = False, vis: bool = False):
    env.reset(seed=seed)
    ...
    _pick_and_place(env, left_planner, right_planner, agent_id=0, obj=env.apple_out, place_center_xyz=apple_basket_center)
    left_planner.move_to_pose_with_screw(left_init_pose, other_gripper_state=right_planner.gripper_state)
    _pick_and_place(env, right_planner, left_planner, agent_id=1, obj=env.banana_out, place_center_xyz=banana_basket_center)
