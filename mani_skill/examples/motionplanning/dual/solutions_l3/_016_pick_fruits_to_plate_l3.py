...
from mani_skill.envs.tasks import TwoRobotPickFruitsToPlateEnvL3
...
def _pick_and_place(
    env: TwoRobotPickFruitsToPlateEnvL3,
    ...
    # Use a per-arm mid pose (lift and retreat towards each arm's side) to reduce collisions.
    lift_pose = grasp_pose * sapien.Pose([0, 0, -0.12])
    place_pose = sapien.Pose(p=place_center_xyz + np.array([0.0, 0.0, 0.12]), q=grasp_pose.q)
...
def solve(env: TwoRobotPickFruitsToPlateEnvL3, seed=None, debug: bool = False, vis: bool = False):
    env.reset(seed=seed)
    ...
    plate_center_xyz = env.plate.pose.sp.p
    ...
    # Enforce: fruit A with left arm, fruit B with right arm.
    _pick_and_place(env, left_planner, right_planner, agent_id=0, obj=env.fruit_a, place_center_xyz=plate_center_xyz + np.array([-0.05, 0.05, 0.0]), vis=vis)
    left_planner.move_to_pose_with_screw(left_init_pose, other_gripper_state=right_planner.gripper_state)
    _pick_and_place(env, right_planner, left_planner, agent_id=1, obj=env.fruit_b, place_center_xyz=plate_center_xyz + np.array([0.05, -0.05, 0.0]), vis=vis)
