import numpy as np
import sapien
from mani_skill.envs.tasks.tabletop.dual_tasks._019_clean_desk import TwoRobotCleanDeskEnv
...
def solve(env: TwoRobotCleanDeskEnv, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    ...
    # -----------------------------
    obb = get_actor_obb(env.tissue, to_world_frame=True, vis=False)
    ...
    goal_pose = env.left_agent.build_grasp_pose(grasp_info["approaching"], grasp_info["closing"], env.brick.pose.sp.p)
    ...
    # -----------------------------
    RADIUS = 0.05
    # wipe_pose_1 = goal_pose * sapien.Pose([-RADIUS, RADIUS/2, 0])
    # wipe_pose_2 = goal_pose * sapien.Pose([-RADIUS, -RADIUS/2, 0])

    wipe_pose_1 = goal_pose * sapien.Pose([-RADIUS, 0, -0.02])
    wipe_pose_2 = goal_pose * sapien.Pose([RADIUS, 0, -0.02])
    ...
    # -----------------------------
    # 4) Back to initial placement region, put down tissue
    # -----------------------------
