...
from mani_skill.envs.tasks.tabletop import TwoRobotTransFoodEnvL3
...
def solve(env: TwoRobotTransFoodEnvL3, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    ...
    # -------------------------------------------------------------------------- #
    # 3. Compute the grasp pose (Pick bowl_src)
    # -------------------------------------------------------------------------- #

    obb = get_actor_obb(env.bowl_src, to_world_frame=True, vis=False)

    # Set the grasp approach direction
    # approaching: [0, 0, -1] grasps from the top down
    ...
    rot_q = euler2quat(0, np.pi, np.pi)
    ...
    dump_rot = euler2quat(-np.pi / 7, 0, 0)  # pouring action
    dump_pose = transport_pose * mirror_offset_pose(sapien.Pose([-0.02, 0, 0], dump_rot))
