...
def solve(env: TwoRobotPlaceFileFolderEnv, seed=None, debug=False, vis=False):
    ...
    base_tray = sapien.Pose(env.tray.pose.sp.p, np.array(left_init_pose.q[0]))
    ...
    reach_pose3 = base_tray * mirror_offset_pose(
        sapien.Pose(p=[0, 0.2, -0.24], q=euler2quat(np.pi / 2, 0, 0))
    )
    goal_pose = base_tray * mirror_offset_pose(
        sapien.Pose(p=[0, 0.1, -0.15], q=euler2quat(np.pi / 6, 0, 0))
    )
    reach_pose4 = base_tray * mirror_offset_pose(
        sapien.Pose(p=[0, 0.2, -0.2], q=euler2quat(np.pi / 8, 0, 0))
    )
