...
def solve(env: TwoRobotPourLiquidCupEnv, seed=None, debug=False, vis=False):
    ...
    # Reach
    # -------------------------------------------------------------------------- #
    reach_pose1 = grasp_pose * mirror_offset_pose(sapien.Pose([0, 0.0, -0.15], euler2quat(0, np.pi/4, -np.pi / 4)))
    if not lr_mirror:
    ...
    # Grasp
    # -------------------------------------------------------------------------- #
    pre_grasp_offset = mirror_offset_pose(sapien.Pose([0.0, 0.01, -0.08], euler2quat(0, np.pi/4, -np.pi/4)))
    pre_grasp_pose = grasp_pose * pre_grasp_offset
    if lr_mirror:
        pre_grasp_pose = pre_grasp_pose * sapien.Pose(q=euler2quat(0, 0, np.pi))
        # Mirror mode only: add a pre-pre-grasp by retreating along pre_grasp_pose local z-axis.
        pre_pre_grasp_pose = pre_grasp_pose * sapien.Pose([0.0, 0.0, -0.1])
        left_planner.move_to_pose_with_screw(pre_pre_grasp_pose, other_gripper_state=right_planner.gripper_state)
