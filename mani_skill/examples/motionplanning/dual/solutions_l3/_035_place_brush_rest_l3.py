...
def solve(env: TwoRobotPlaceBrushRestEnv, seed=None, debug=False, vis=False):
    ...
    reach_pose2 = grasp_pose * sapien.Pose([-0.03, 0.0, -0.2])

    # -------------------------------------------------------------------------- #
    # Reach
    # -------------------------------------------------------------------------- #
    reach_pose1 = grasp_pose * sapien.Pose([-0.03, 0, -0.15])
    left_planner.move_to_pose_with_screw(reach_pose1, other_gripper_state=right_planner.gripper_state)

    # -------------------------------------------------------------------------- #
    # Grasp
    # -------------------------------------------------------------------------- #
    left_planner.move_to_pose_with_screw(grasp_pose * sapien.Pose([0.0, 0.0, 0], euler2quat(0, np.pi/4, 0)), other_gripper_state=right_planner.gripper_state)
    ...
    reach_pose3 = goal_pose * sapien.Pose([0.0, 0.0, -0.4], euler2quat(0, 0, 0))
    left_planner.move_to_pose_with_screw(reach_pose3, other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(goal_pose * sapien.Pose([-0.05, 0.0, -0.35], euler2quat(0, -np.pi/4, 0)), other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(goal_pose * sapien.Pose([-0.05, 0.0, -0.2], euler2quat(0, -np.pi/4, 0)), other_gripper_state=right_planner.gripper_state)
