...
from mani_skill.envs.tasks import TwoRobotPlaceScrewdriverEnvL3
...
def solve(env: TwoRobotPlaceScrewdriverEnvL3, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    ...
    goal_pose = sapien.Pose(env.cube.pose.sp.p, grasp_pose.q) * sapien.Pose([0, 0, -0.05])
    reach_pose2 = grasp_pose * sapien.Pose([-0.01, 0.02, -0.2])

    # -------------------------------------------------------------------------- #
    # Reach
    # -------------------------------------------------------------------------- #
    reach_pose1 = grasp_pose * sapien.Pose([-0.01, 0.02, -0.15])
    left_planner.move_to_pose_with_screw(reach_pose1, other_gripper_state=right_planner.gripper_state)

    # -------------------------------------------------------------------------- #
    # Grasp
    # -------------------------------------------------------------------------- #
    left_planner.move_to_pose_with_screw(grasp_pose * sapien.Pose([-0.01, 0.02, 0.05]), other_gripper_state=right_planner.gripper_state)
    ...
    reach_pose3 = goal_pose * sapien.Pose([0.0, 0.13, -0.10], euler2quat(0, 0, -np.pi/2))
    left_planner.move_to_pose_with_screw(reach_pose3, other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(goal_pose * sapien.Pose([0.0, 0.13, -0.05], euler2quat(0, 0, -np.pi/2)), other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(reach_pose3, other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(reach_pose2, other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(grasp_pose * sapien.Pose([-0.01, 0.02, 0.05]), other_gripper_state=right_planner.gripper_state)
