...
from mani_skill.envs.tasks import TwoRobotPourCupEnvL3
...
def solve(env: TwoRobotPourCupEnvL3, seed=None, debug=False, vis=False):
    """
    Phases:
    1. Grasp kettle
    2. Lift kettle
    ...
    above_cup1_pose_base = sapien.Pose(
        env.cup1.pose.sp.p + mirror_xyz(np.array([0, -0.1, 0.25], dtype=np.float32)),
        grasp_pose.q
    )
    backward_offset = sapien.Pose([0, 0, 0])
    ...
    #     cup2_initial_pos + np.array([0, 0, 0.0]), 
    #     grasp_pose.q
    # )
    # left_planner.move_to_pose_with_screw(
    #     return_above_pose,
    ...
    place_pose = sapien.Pose(
        cup2_initial_pos + mirror_xyz(np.array([0, 0, 0.05], dtype=np.float32)),
        grasp_pose.q
    )
    left_planner.move_to_pose_with_screw(
        place_pose,
