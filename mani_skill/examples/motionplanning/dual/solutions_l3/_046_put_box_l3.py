import numpy as np
import sapien
from mani_skill.envs.tasks import TwoRobotPutBoxEnvL3
...
def solve(env: TwoRobotPutBoxEnvL3, seed=None, debug=False, vis=False):
    """
    ...
    # Lift Up
    tennis_lift_pose = tennis_grasp_pose * sapien.Pose([0, 0, -0.20])
    left_planner.move_to_pose_with_screw(
    ...
    place_position = box_pose.p + np.array([-0.1, 0, 0.20]) 
    ...
    high_pose = sapien.Pose(
        p=box_pose.p + np.array([0, 0, 0.20]),
    ...
    push_start_pos = box_pose.p + np.array([0.10, 0, 0.145]) # Height 12cm
    ...
    push_start_high = sapien.Pose(
        p=box_pose.p + np.array([0.10, 0, 0.25]),
        q=tennis_grasp_pose.q
    )
    left_planner.move_to_pose_with_screw(
