...
from mani_skill.envs.tasks import TwoRobotPressJuicerEnvL3
from mani_skill.examples.motionplanning.panda.motionplanner import \
    PandaArmMotionPlanningSolver
from mani_skill.examples.motionplanning.panda.utils import (
    compute_grasp_info_by_obb, get_actor_obb)

def solve(env: TwoRobotPressJuicerEnvL3, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    ...
    goal_pose = sapien.Pose(env.bowl.pose.sp.p, grasp_pose.q) * sapien.Pose([0, 0, -0.05])

    # -------------------------------------------------------------------------- #
    # Reach
    # -------------------------------------------------------------------------- #
    reach_pose1 = grasp_pose * sapien.Pose([0, 0.15, -0.15], euler2quat(0, 0, -np.pi / 2))
    ...
    left_planner.move_to_pose_with_screw(
        grasp_pose * sapien.Pose([0, 0, -0.1], q=euler2quat(np.pi / 2, np.pi / 2, 0)),
    ...
    left_planner.move_to_pose_with_screw(
        grasp_pose * sapien.Pose([0, 0, -0.2], q=euler2quat(np.pi / 2, np.pi / 2, 0)),
        other_gripper_state=right_planner.gripper_state,
    )
    ...
    # Move to goal pose
    # -------------------------------------------------------------------------- #
    reach_pose3 = goal_pose * sapien.Pose([0.0, -0.02, -0.2], q=euler2quat(np.pi / 2, np.pi / 2, 0))
    left_planner.move_to_pose_with_screw(reach_pose3, other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(
        goal_pose * sapien.Pose([0.0, -0.02, -0.1], q=euler2quat(np.pi / 2, np.pi / 2, 0)),
        other_gripper_state=right_planner.gripper_state,
    )
    left_planner.move_to_pose_with_screw(reach_pose3, other_gripper_state=right_planner.gripper_state)
    left_planner.move_to_pose_with_screw(
