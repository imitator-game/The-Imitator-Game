...
class PandaArmMotionPlanningSolver:
    def __init__(
        ...
        if self.vis and self.visualize_target_grasp_pose:
            ...
            if hasattr(self.base_env, "right_agent"):
                self.grasp_pose_visual.set_pose(self.base_env.right_agent.tcp.pose)
            if multi_robot_id is not None:
    ...
    def move_through_waypoints_smooth(
            self,
            poses: List[Union[sapien.Pose, np.ndarray]],
            dry_run: bool = False,
            refine_steps: int = 0,
        ...
        """
        Move smoothly through multiple waypoint poses with unified acceleration/deceleration

        Args:
            poses: List of poses to visit
            dry_run: If True, only plan but don't execute
            refine_steps: Additional steps to execute at the end
            points_per_segment: Number of interpolation points between waypoints
            use_straight_line: If True, maintain straight line motion between poses
        """
        ...
        for pose in poses:
            if isinstance(pose, sapien.Pose):
                pose = to_sapien_pose(pose)
                waypoint_poses.append(np.concatenate([pose.p, pose.q]))
            else:
                waypoint_poses.append(pose)

        # Visualize final pose if enabled
        if self.grasp_pose_visual is not None and waypoint_poses:
            final_pose = sapien.Pose(p=waypoint_poses[-1][:3], q=waypoint_poses[-1][3:])
            self.grasp_pose_visual.set_pose(final_pose)

        # Plan smooth trajectory through all waypoints
        result = self.planner.plan_waypoints_smooth(
        ...
        if result["status"] != "Success":
            print(f"Smooth waypoint planning failed: {result['status']}")
            self.render_wait()
            return -1
    ...
    def open_gripper(self, other_gripper_state=None, scale=1.0):
        self.gripper_state = OPEN
        qpos = self.robot.get_qpos()[0, :-2].cpu().numpy()
        for i in range(6):
            if self.control_mode == "pd_joint_pos":
                action = np.hstack([qpos, self.gripper_state * scale])
            else:
                action = np.hstack([qpos, qpos * 0, self.gripper_state * scale])
            if self.multi_robot_id is not None:
                action = np.hstack([qpos, self.gripper_state * scale])
                other_action = np.hstack([self.env.agent.agents[1-self.multi_robot_id].robot.qpos[0][:7], other_gripper_state])
                action = OrderedDict({
    ...
    def add_collision_pts(self, pts: np.ndarray):
        self.use_point_cloud = True  # Enable point cloud collision detection
        if self.all_collision_pts is None:
            self.all_collision_pts = pts
        else:
            self.all_collision_pts = np.vstack([self.all_collision_pts, pts])
