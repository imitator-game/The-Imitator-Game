...
from mani_skill.examples.motionplanning.panda.solutions import (solvePushCube, solvePickCube, solveStackCube, solvePegInsertionSide, solvePlugCharger,
                       # solveStackCubeGraspNet,
                       solvePullCubeTool, solveLiftPegUpright, solvePullCube, solvePickPlace,
                        solveDrawTriangle, solveDrawSVG, solveFoldSuitcase,
                        solvePickCubeYCB, solveStackPyramid, solvePandaMicrowave)
MP_SOLUTIONS = {
    "DrawTriangle-v1": solveDrawTriangle,
    "PickCube-v1": solvePickCube,
    "StackCube-v1": solveStackCube,
    # "StackCube-GraspNet-v1": solveStackCubeGraspNet,
    ...
    "PickCubeYCB-v1": solvePickCubeYCB,
    "StackPyramid-v1": solveStackPyramid,
    "PandaMicrowave-v1": solvePandaMicrowave,
    "PickPlace-v1": solvePickPlace,
}
def parse_args(args=None):
    ...
    parser.add_argument("--cam-width", type=int, default=None, help="Override camera sensor width (pixels)")
    parser.add_argument("--cam-height", type=int, default=None, help="Override camera sensor height (pixels)")
    ...
    parser.add_argument("--graspnet-checkpoint", type=str, help="Path to GraspNet checkpoint file. Only used for StackCube-GraspNet-v1 environment.")
    return parser.parse_args()

def _main(args, proc_id: int = 0, start_seed: int = 0) -> str:
    env_id = args.env_id

    # Determine observation mode: use pointcloud for GraspNet variant
    obs_mode = args.obs_mode
    if env_id == "StackCube-GraspNet-v1":
        if obs_mode == "none":
            obs_mode = "pointcloud"  # GraspNet requires point cloud data
        actual_env_id = "StackCube-v1"  # Use the actual environment ID
    else:
        actual_env_id = env_id

    # Prepare sensor configs with optional camera resolution override
    ...
    while True:
        # try:
        # Pass graspnet_checkpoint parameter for GraspNet variant
        if env_id == "StackCube-GraspNet-v1":
            res = solve(env, seed=seed, debug=True, vis=True if args.vis else False, graspnet_checkpoint=args.graspnet_checkpoint)
        elif env_id == "PandaMicrowave-v1":
