...
class WidowXAIArmMotionPlanningSolver:
    def __init__(
        self,
        env: BaseEnv,
        debug: bool = False,
        ...
        if self.vis and self.visualize_target_grasp_pose:
            if "grasp_pose_visual" not in self.base_env.scene.actors:
                self.grasp_pose_visual = build_widowxai_gripper_grasp_pose_visual(
                    self.base_env.scene
                )
            else:
                self.grasp_pose_visual = self.base_env.scene.actors["grasp_pose_visual"]
            # Set proper initial pose to avoid warning - convert tensor to numpy
            tcp_pose = self.base_env.agent.tcp.pose
            initial_pose = sapien.Pose(
                p=tcp_pose.p.cpu().numpy()[0], 
                q=tcp_pose.q.cpu().numpy()[0]
            )
            self.grasp_pose_visual.set_pose(initial_pose)
    ...
    def follow_path(self, result, refine_steps: int = 0):
        ...
        for i in range(n_step + refine_steps):
            ...
            if self.control_mode == "pd_joint_pos_vel":
                ...
                action = np.hstack([target_qpos, qvel, self.gripper_state, self.gripper_state])
            elif self.control_mode == "pd_joint_pos":
                action = np.hstack([target_qpos, self.gripper_state, self.gripper_state])
            else:
                action = np.hstack([target_qpos, self.gripper_state, self.gripper_state])
    ...
    def move_to_pose_with_screw(
        ...
        # For WidowXAI, directly use RRTConnect instead of screw planning
        return self.move_to_pose_with_RRTConnect(pose, dry_run, refine_steps)

    def open_gripper(self):
        self.gripper_state = OPEN
        qpos = self.robot.get_qpos()[0, :-2].cpu().numpy()
        for i in range(4):  # Slightly fewer steps (was 6)
    ...
    def close_gripper(self, t=5, gripper_state=CLOSED):  # Slightly fewer steps (was 6)
...
def build_widowxai_gripper_grasp_pose_visual(scene: ManiSkillScene):
    builder = scene.create_actor_builder()
    # Set proper initial pose for the actor builder to avoid warning
    initial_pose = sapien.Pose(p=[0, 0, 0.5], q=[1, 0, 0, 0])  # Position above ground
