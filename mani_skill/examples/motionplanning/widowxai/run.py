...
# Force the use of the development version of ManiSkill (must be before all imports)
...
# Add the current directory to the Python path to import solutions
...
# Import environments - this automatically registers all environments, including our PickCubeYCB-v1
import mani_skill.envs
import gymnasium as gym

# Import solutions
from solutions import solvePickCubeYCB
...
def parse_args(args=None):
    ...
    parser.add_argument("-e", "--env-id", type=str, default="PickCubeYCB-v1", help=f"Environment to run motion planning solver on. Available options are {list(MP_SOLUTIONS.keys())}")
    parser.add_argument("-o", "--obs-mode", type=str, default="none", help="Observation mode to use. Usually this is kept as 'none' as observations are not necessary to be stored, they can be replayed later via the mani_skill.trajectory.replay_trajectory script.")
...
def _main(args, proc_id: int = 0, start_seed: int = 0) -> str:
    ...
    env = gym.make(
        ...
        robot_uids="widowxai",  # Specify WidowXAI robot
    )
    ...
    print(f"Motion Planning Running on {env_id} with WidowXAI")
