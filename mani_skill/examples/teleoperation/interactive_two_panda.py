...
import mani_skill.envs.tasks.tabletop.dual_tasks._006_pick_wash
import mani_skill.envs.tasks.tabletop.dual_tasks._019_clean_desk
import mani_skill.envs.tasks.tabletop.dual_tasks._022_trans_food
import mani_skill.envs.tasks.tabletop.dual_tasks._046_put_box
...
@dataclass
class Args:
    env_id: Annotated[str, tyro.conf.arg(aliases=["-e"])] = "TwoRobotFold-v1"
    obs_mode: str = "none"
...
def solve(env: BaseEnv, debug=False, vis=False):
    ...
    def select_panda_hand():
        viewer.select_entity(sapien_utils.get_obj_by_name(env.agent.agents[arm_to_control].robot.links, "panda_hand")._objs[0].entity)
    select_panda_hand()
    ...
    while True:
        ...
        #     pass
            ...
            if gripper_open:
                gripper_open = False
                if arm_to_control == 0:
                    _, reward, _, _, info = planner.close_gripper(right_planner.gripper_state)
                else:
                    _, reward, _, _, info = planner.close_gripper(left_planner.gripper_state)
            else:
                gripper_open = True
                if arm_to_control == 0:
                    _, reward, _, _, info = planner.open_gripper(right_planner.gripper_state)
                else:
                    _, reward, _, _, info = planner.open_gripper(left_planner.gripper_state)
        ...
        if execute_current_pose:
            ...
            if result != -1 and len(result["position"]) < 150:
                if arm_to_control == 0:
                    _, reward, _ ,_, info = planner.follow_path(result, right_planner.gripper_state)
                else:
                    _, reward, _, _, info = planner.follow_path(result, left_planner.gripper_state)
                print(f"Reward: {reward}, Info: {info}")
            else:
