...
def load_actions_from_h5(traj_group):
    """Load actions from HDF5 trajectory group, supporting both single and multi-robot cases.

    Args:
        traj_group: HDF5 group containing trajectory data

    Returns:
        numpy array for single robot, OrderedDict for multi-robot
    """
    from collections import OrderedDict

    actions = traj_group["actions"]

    # Check if actions is a dataset (single robot) or group (multi-robot)
    if isinstance(actions, h5py.Dataset):
        # Single robot case
        return np.array(actions)
    elif isinstance(actions, h5py.Group):
        # Multi-robot case - return OrderedDict
        action_dict = OrderedDict()
        for robot_name in actions.keys():
            action_dict[robot_name] = np.array(actions[robot_name])
        return action_dict
    else:
        raise ValueError(f"Unknown actions type: {type(actions)}")
...
def replay_parallelized_sim(
    ...
    for episode_batch_index, episode_batch in enumerate(batches):
        ...
        for i, trajectory_id in enumerate(trajectory_ids):
            ...
            actions = load_actions_from_h5(traj)
            ...
            if isinstance(actions, dict):
                # Multi-robot case
                for robot_name in actions.keys():
                    if len(actions[robot_name]) < episode_batch_max_len:
                        actions[robot_name] = np.concatenate(
                            [
                                actions[robot_name],
                                np.zeros(
                                    (episode_batch_max_len - len(actions[robot_name]), actions[robot_name].shape[1])
                                ),
                            ],
                            axis=0,
                        )
        ...
        if isinstance(original_actions_batch[0], dict):
            # Multi-robot case: stack each robot's actions separately
            from collections import OrderedDict
            stacked_actions = OrderedDict()
            robot_names = original_actions_batch[0].keys()
            for robot_name in robot_names:
                robot_actions = [actions[robot_name] for actions in original_actions_batch]
                stacked_actions[robot_name] = np.stack(robot_actions, axis=1)
            original_actions_batch = stacked_actions
        else:
            # Single robot case
...
def replay_cpu_sim(
    ...
    for episode in episodes:
        ...
        is_multi_robot = isinstance(ori_control_mode, dict)
        if is_multi_robot:
            control_modes = list(ori_control_mode.values())
            if not all(mode == control_modes[0] for mode in control_modes):
                raise ValueError(
                    f"All robots must use the same control mode for conversion, but got: {ori_control_mode}"
                )
            unified_ori_control_mode = control_modes[0]
        else:
            unified_ori_control_mode = ori_control_mode
        if pbar is not None:
        ...
        for _ in range(args.max_retry + 1):
            ...
            ori_actions = load_actions_from_h5(trajectories[traj_id])
            info = {}
            ...
            if (
                args.target_control_mode is None
                or unified_ori_control_mode == args.target_control_mode
            ):
                n = len(ori_actions)
                if pbar is not None:
                    pbar.reset(total=n)
            ...
            # From joint position to others
            elif unified_ori_control_mode == "pd_joint_pos":
                info = action_conversion.from_pd_joint_pos(
                    ...
                    ori_env_states=ori_env_states,
                )

            # From joint delta position to others
            elif unified_ori_control_mode == "pd_joint_delta_pos":
