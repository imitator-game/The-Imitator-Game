...
def from_pd_joint_pos(
    ...
    is_multi_robot = isinstance(ori_actions, dict)

    if is_multi_robot:
        if "ee" in output_mode:
            raise NotImplementedError(
                "Converting from pd_joint_pos to ee control modes is not yet supported for multi-robot environments"
            )

        from collections import OrderedDict

        n = len(next(iter(ori_actions.values())))
        if pbar is not None:
            pbar.reset(total=n)
        ...
        robot_names = list(ori_actions.keys())
        ...
        for t in range(n):
            ...
            ori_action_t = OrderedDict()
            for robot_name in robot_names:
                ori_action_t[robot_name] = ori_actions[robot_name][t]

            # Convert to tensor and get action dict
            ori_action_t = common.to_tensor(ori_action_t, device=ori_env.device)

            ori_action_dict = OrderedDict()
            for robot_name in robot_names:
                ori_action_dict[robot_name] = ori_controller[robot_name].to_action_dict(ori_action_t[robot_name])

            output_action_dict = ori_action_dict.copy()
            output_action = OrderedDict()

            # Step the original environment first
                ...
                for robot_name in robot_names:
                    if output_mode == "pd_joint_delta_pos":
                        arm_action = qpos_to_pd_joint_delta_pos(
                            controller[robot_name].controllers["arm"], ori_action_dict[robot_name]["arm"]
                        )
                    ...
                    if np.max(np.array(np.abs(arm_action))) > 1 + 1e-3:
                        if verbose:
                            tqdm.write(f"Arm action is clipped: {arm_action}")
                        flag = False
                    arm_action = np.clip(arm_action, -1, 1)
                    output_action_dict[robot_name]["arm"] = arm_action

                    output_action[robot_name] = controller[robot_name].from_action_dict(
                        common.to_tensor(output_action_dict[robot_name], device=env.device)
                    )

                output_action_tensor = torch.cat(list(output_action.values()))
                obs, _, _, _, info = env.step(output_action_tensor)
                if use_env_states:
