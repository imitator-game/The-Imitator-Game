...
def _load_partnet_mobility_dataset(mode):
    ...
    PARTNET_MOBILITY = {
        "model_data": load_json(
            PACKAGE_ASSET_DIR / f"partnet_mobility/meta/partnet_info/info_{mode}.json"
        ),
    }

    def find_urdf_path(model_id):
        model_dir = ASSET_DIR / f"partnet_mobility/dataset/{mode.capitalize()}" / str(model_id)
        if mode in ['cabinet', 'drawer']:
            model_dir = ASSET_DIR / f"partnet_mobility/dataset/Storagefurniture" / str(model_id)
        # urdf_names = ["mobility_cvx.urdf", "mobility_fixed.urdf"]
        if mode in ['kitchenpot']:
            model_dir = ASSET_DIR / f"partnet_mobility/dataset/KitchenPot" / str(model_id)
        if mode in ['coffeemachine']:
            model_dir = ASSET_DIR / f"partnet_mobility/dataset/CoffeeMachine" / str(model_id)
