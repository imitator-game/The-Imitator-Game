...
class AI2THORBaseSceneBuilder(SceneBuilder):
    ...
    def initialize(self, env_idx):
        ...
        if self.env.robot_uids == "fetch":
            ...
            agent: Realman = self.env.agent
            base_qpos = torch.zeros(len(agent.robot.get_active_joints()))
            base_qpos[0:2] = torch.tensor([0.0, 0.0])  # head joints
            ...
            for i in range(7):
                base_qpos[2 + i * 2] = right_arm_qpos[i]
                base_qpos[2 + i * 2 + 1] = left_arm_qpos[i]

            for i in range(16, len(base_qpos)):
                base_qpos[i] = 0.0

            qpos = base_qpos.repeat(1).reshape(1, -1)
            if self.robot_init_qpos_noise > 0:
                arm_noise = torch.randn(1, 14) * self.robot_init_qpos_noise
                qpos[:, 2:16] += arm_noise

            agent.robot.set_qpos(qpos)

            # Set head joint properties
            head_joints = [
                agent.robot.get_active_joints()[0],
                agent.robot.get_active_joints()[1]
            ]
            ...
            # For the purposes of physical simulation, we disable collisions between the Fetch robot and the scene background
            self.disable_fetch_move_collisions(self.bg)
        elif self.env.robot_uids == "realman_mobile_base":
            agent: RealmanMobileBase = self.env.agent
            base_qpos = torch.zeros(len(agent.robot.get_active_joints()))

            # Mobile base joints (first 3)
            base_qpos[0:3] = torch.tensor([0.0, 0.0, 0.0])  # root_x, root_y, root_z_rotation

            # Head joints
            base_qpos[3:5] = torch.tensor([0.0, 0.0])  # head_joint1, head_joint2

            # Arm joints (following the alternating pattern in the joint list)
            right_arm_qpos = torch.tensor([0.82383, -0.17647, 0, -1.44114, -2.09, 0.926211, 1.55079])
            left_arm_qpos = torch.tensor([-0.81842, -0.176488, 0, -1.37455, -0.875422, -0.997037, -1.54696])

            # Set arm joints according to the actual joint order
            # r_joint1 at index 5, l_joint1 at index 6, r_joint2 at index 7, l_joint2 at index 8, etc.
            for i in range(7):
                base_qpos[5 + i * 2] = right_arm_qpos[i]  # r_joint[i+1]
                base_qpos[6 + i * 2] = left_arm_qpos[i]  # l_joint[i+1]

            # Gripper joints (indices 19-26)
            for i in range(19, len(base_qpos)):
                base_qpos[i] = 0.04

            qpos = base_qpos.repeat(1).reshape(1, -1)

            # Add noise to arm joints if specified
            if self.robot_init_qpos_noise > 0:
                arm_noise = torch.randn(1, 14) * self.robot_init_qpos_noise
                qpos[:, 5:19] += arm_noise  # Arms are at indices 5-18

            agent.robot.set_qpos(qpos)

            # Set head joint properties to keep them fixed
            head_joints = [
                agent.robot.get_active_joints()[3],  # head_joint1
                agent.robot.get_active_joints()[4]  # head_joint2
            ]
            for joint in head_joints:
                joint.set_drive_properties(
                    stiffness=10000,
