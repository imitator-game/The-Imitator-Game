...
class Point:
    points: list["Point"] = []
    """A point in a specific base coordinate frame"""
    ...
    def base2world(self, entity_mat, scale=1.0) -> sapien.Pose:
        """Convert a matrix in the base coordinate frame to the world coordinate frame"""
    ...
    def word2base(self, entity_mat, scale=1.0) -> sapien.Pose:
        """Convert a matrix in the world coordinate frame to the base coordinate frame"""
        entity_mat = np.array(entity_mat)
        base_mat = self.base.get_pose().to_transformation_matrix()
...
def rotate_along_axis(
    ...
    """
    Rotate by a given angle about the specified axis centered on center. The rotation
    direction can be controlled via towards (the direction where the product of the
    center->target vector and the towards vector is positive).

    target_pose: the target point (e.g. a pre-grasp point directly above the object)
    center_pose: the center point (e.g. the object position)
    axis: the rotation axis
    theta: the rotation angle (in radians)
    axis_type: the type of rotation axis ('center': relative to center_pose,
        'target': relative to target_pose, 'world': world coordinate frame), default is 'center'
    towards: the rotation direction (optional); if given, the rotation direction is
        determined according to this parameter
    camera_face: the camera facing direction (optional), constrains the camera vector
        to have a positive dot product with this vector; if set to None, only rotate
        without considering the camera facing
    Returns: a list whose first 3 elements are the coordinates and last 4 elements are the quaternion
    """
...
def rotate2rob(target_pose, rob_pose, box_pose, theta: float = 0.5) -> list:
    """
    Offset toward the specified rob_pose
    """
...
def get_align_matrix(v1: np.ndarray, v2: np.ndarray) -> np.ndarray:
    """
    Get the rotation matrix that aligns v1 to v2
    """
...
def get_product_vector(v1: np.ndarray, v2: np.ndarray) -> np.ndarray:
    """
    Get the projection vector of v2 onto v1
    """
    v1 = np.array(v1).reshape(3)
    v1 = v1 / np.linalg.norm(v1)
    v2 = np.array(v2).reshape(3)
...
def get_place_pose(
    ...
    Considerations:
        1. The 3D coordinates match the given coordinates
        2. A reasonable object orientation:
            - The object's z axis aligns with the given coordinate's z axis
            - Satisfy certain constraints on the xy plane:
                - No constraint (directly use the projection of the object's current
                  x, y onto the xOy plane)
                - The object's x axis aligns with the given x axis
                - Select the direction along which the dot product of the object's
                  x axis and the given world axis unit vector set is minimized

    actor_pose: the current pose of the object
    target_pose: the pose where the object should be placed
    constrain: the constraint type of the object
        - free: no constraint
        - align: the direction along which the dot product of the object's x axis
          and the given world axis vector set is minimized
    align_axis: the given set of world axis vectors; if None, default to target_pose's x axis
    actor_axis: the actor axis used for the dot product, x axis by default
    actor_axis_type: the type of actor_axis, local coordinate frame by default
        - actor: the local coordinate frame of actor_pose
        - world: the world coordinate frame
    """
    actor_pose_mat = _toPose(actor_pose).to_transformation_matrix()
    target_pose_mat = _toPose(target_pose).to_transformation_matrix()

    # Align the object's 3D coordinates with the given coordinates
    ...
    # Align the object's z axis with the given coordinate's z axis
