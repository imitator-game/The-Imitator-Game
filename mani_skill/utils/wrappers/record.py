...
# Mirror recording behavior:
# - None: auto-follow env flag `_lr_mirror_applied_this_reset`
# - True/False: force override for debugging.
APPLY_MIRROR = None
...
class RecordEpisode(gym.Wrapper):
    ...
    def __init__(
        ...
        self._apply_mirror_recording = False
    ...
    def _filter_video_info(self, scalar_info):
        whitelist = getattr(self.base_env, "video_info_whitelist", None)
        if whitelist is None:
            return scalar_info
        return {k: v for k, v in scalar_info.items() if k in whitelist}
    ...
    def reset(
        ...
        **kwargs,
        ...
        if APPLY_MIRROR is None:
            self._apply_mirror_recording = bool(
                getattr(self.base_env, "_lr_mirror_applied_this_reset", False)
            )
        else:
            self._apply_mirror_recording = bool(APPLY_MIRROR)
        if info["reconfigure"]:
    ...
    def step(self, action):
        ...
        if self.save_video:
            ...
            if self.info_on_video:
                ...
                scalar_info = self._filter_video_info(scalar_info)
        ...
        for env_idx in env_idxs_to_flush:
            ...
            if save:
                ...
                def recursive_add_to_h5py(
                    ...
                    mapping = {
                        'panda_wristcam-0': 'panda_wristcam-1',
                        'panda_wristcam-1': 'panda_wristcam-0',
                        'panda_wristcam-agent-0': 'panda_wristcam-agent-1',
                        'panda_wristcam-agent-1': 'panda_wristcam-agent-0'
                    }
                    if self._apply_mirror_recording:
                        key = mapping.get(key, key)
                    if isinstance(data, dict):
                        if key in ['actions']:
                            if self._apply_mirror_recording:
                                actions = np.concatenate([data[mapping[k]] for k in data.keys()], axis=1)
                            else:
                                actions = np.concatenate(list(data.values()), axis=1)
                ...
                episode_info = dict(
                    ...
                    lr_mirror_applied=bool(self._apply_mirror_recording),
                )
                if self.num_envs == 1:
